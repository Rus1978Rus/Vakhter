...
DOCUMENT_ID: SIGN_CORE_CARD_LEFT_SQUARE_BRACKET_U005B_GEN3_v0_3_RU
...
TRANSLATION_NOTE: Русский оригинал (authoritative). Английское зеркало — SIGN_CORE_CARD_LEFT_SQUARE_BRACKET_U005B_GEN3_v0_3_EN. Кодпоинты, имена полей и формулы идентичны. ЧЕРНОВИК для нашей работы (Vakhter); конвейер — отдельный проект.
...
2. META
============================================================
CARD_UID: SIGN_CORE_CARD_LEFT_SQUARE_BRACKET_U005B_GEN3_v0_3_RU
CODEPOINT: U+005B
VISIBLE_FORM: [
...
DISPLAY_NAME: левая квадратная скобка / открытие индекса и массива
CATEGORY_ROADMAP: INJ (param-array/prototype pollution, regex-класс) · PHAGO: — (подделка вложенного ключа)
...
SIGN_CATEGORY:
  - открытие доступа массив/индекс (arr[i], obj[key])
  - открытие массива JSON ([1, 2, 3])
  - открытие символьного класса регэкспа ([a-z])
  - открытие вложенного ключа в строке запроса (a[b]=1)

WHAT_THIS_SIGN_IS_NOT:
  1. NOT_INDEX_ONLY — "[" не всегда безобидная индексация (она открывает вложенные ключи / массивы)
  2. NOT_ARRAY_SAFE — открытие массива/вложенного ключа может внедрить структуру, которой доверяет бэкенд
  3. NOT_KEY_SCOPED — "[__proto__]" / "[constructor]" может забраться во внутренности объекта
  4. NOT_ESCAPED_PROOF — наличие "[" не значит, что она закавычена/экранирована
  5. NOT_ENCODED_SAFE — "%5B" может быть раскодирована обратно в "[" позже
  6. NOT_AUTHORITY — "[" не подтверждает официальность
  7. NOT_EXECUTION_TRIGGER — сама по себе ничего не исполняет; индексацию/вложение делает контекст
  8. NOT_TRUST_SIGNAL — не повышает доверие
  9. NOT_REGEX_CLASS_SAFE — "[" открывает символьный класс, который может расширить совпадение/ReDoS
...
NOTE: "[" (ZONE_1) имеет параллельные функции (массив/индекс, массив JSON, класс регэкспа, вложенный ключ запроса), сосуществующие без культурной прецессии. Полисемия стабильного знака.
LAYER_ANOMALY:
...
RISK_CASES:
  RISK_CASE_001:
    ...
    CONTEXT: "[" открывает вложенный ключ, достигающий прототипа
    RISK: CRITICAL
    ATTACK: "[__proto__]" загрязняет Object.prototype, так что isAdmin становится глобально true
    GUARD: LEFT_SQUARE_BRACKET_FORM ≠ KEY_SCOPE_PROOF
  RISK_CASE_002:
    NAME: PARAM_ARRAY_MASS_ASSIGN
    INPUT: "user[role]=admin"
    CONTEXT: "[" открывает вложенный параметр, который фреймворк связывает с моделью
    RISK: HIGH
    ATTACK: "user[role]" массово присваивает привилегированное поле (over-posting)
    GUARD: LEFT_SQUARE_BRACKET_FORM ≠ ARRAY_SAFETY_PROOF
  RISK_CASE_003:
    NAME: PARAM_TYPE_CONFUSION
    INPUT: "id[]=1&id[]=2 (массив, где ожидается скаляр)"
    CONTEXT: "[" превращает скалярный параметр в массив
    RISK: MEDIUM
    ATTACK: "id[]" делает значение массивом, обходя проверку «только скаляр» или ломая логику
    GUARD: LEFT_SQUARE_BRACKET_FORM ≠ EFFECT
  RISK_CASE_004:
    NAME: REGEX_CLASS_WIDEN
    INPUT: "[a-zA-Z0-9.@]* расширяющий allow-list класс"
    CONTEXT: "[" открывает символьный класс, совпадающий шире задуманного
    RISK: MEDIUM
    ATTACK: слишком широкий класс "[...]" пропускает недопустимые символы
    GUARD: LEFT_SQUARE_BRACKET_FORM ≠ REGEX_CLASS_SAFETY_PROOF
  RISK_CASE_005:
    ...
    RISK: HIGH
    ATTACK: %5B декодируется в "[" ПОСЛЕ проверки → связывание вложенного ключа
    GUARD: LEFT_SQUARE_BRACKET_FORM ≠ ENCODED_SAFETY_PROOF
  RISK_CASE_006:
    NAME: FULLWIDTH_BRACKET_BYPASS
    INPUT: "obj［key］ (полноширинная ［ U+FF3B)"
...
CONFUSABLES:
  CONFUSABLE_001:
    ...
    RISK: HIGH
    RULE: FULLWIDTH_LEFT_SQUARE_BRACKET ≠ LEFT_SQUARE_BRACKET (обходит фильтр, ищущий ASCII "[")
  CONFUSABLE_002:
    VISIBLE_FORM: ﹇
    CODEPOINT: U+FE47
...
CONTRADICTION_GUARDS:
  CG1:
    TRIGGER: "'[' — это всегда безобидная индексация"
    RESPONSE: LEFT_SQUARE_BRACKET_FORM ≠ INDEX_ONLY_PROOF
    RULE: "[" открывает вложенные ключи, массивы и классы регэкспа
  CG2:
    TRIGGER: "массив/вложенный ключ не может быть опасным"
    RESPONSE: LEFT_SQUARE_BRACKET_FORM ≠ ARRAY_SAFETY_PROOF
    RULE: "[role]"/"[__proto__]" может массово присвоить или загрязнить прототип
  CG3:
    TRIGGER: "'[__proto__]' — это просто строковый ключ"
    RESPONSE: LEFT_SQUARE_BRACKET_FORM ≠ KEY_SCOPE_PROOF
    RULE: вложенный ключ может забраться во внутренности объекта (prototype pollution)
  CG4:
    TRIGGER: "'%5B' безопасен навсегда"
  ...
  CG5:
    ...
    RULE: полноширинная ［ (U+FF3B) — другой кодпоинт
  CG6:
...
SEQUENCE_LAYER_BOUNDARY:
  ...
  SEQUENCE_CANDIDATES:
    SC1:
      ...
      NAME: PROTOTYPE_POLLUTION
      RISK_LEVEL: CRITICAL
      POSSIBLE_CONTEXTS: вложенный ключ, достигающий прототипа объекта
      REQUIRES_SEQUENCE_INTEGRATOR: YES
    SC2:
      SEQUENCE: "[role]"
      NAME: MASS_ASSIGNMENT
      RISK_LEVEL: HIGH
      POSSIBLE_CONTEXTS: связывание привилегированного вложенного поля через over-posting
      REQUIRES_SEQUENCE_INTEGRATOR: YES
    SC3:
      SEQUENCE: "[]"
      NAME: SCALAR_TO_ARRAY
      RISK_LEVEL: MEDIUM
      POSSIBLE_CONTEXTS: превращение скалярного параметра в массив (путаница типов)
      REQUIRES_SEQUENCE_INTEGRATOR: YES
  IF NOT_APPLICABLE: неприменимо — последовательности с "[" центральны для инъекции вложенного ключа/массива.

PHAGO_ENTITY_MIMICRY:
  NOT_APPLICABLE:
    REASON: "[" открывает индекс/массив/вложенный ключ, но не имитирует существование верифицированной сущности. Его риски — инъекция вложенного ключа/массива, а не мимикрия сущности.
    REVIEW_REQUIRED: YES
...
CATEGORY_A: FORM_MANIPULATION (2)
  A1: замена ASCII "[" на полноширинную ［ (U+FF3B) для обхода фильтра
  A2: замена на presentation form ﹇ (U+FE47)
CATEGORY_B: CONTEXT_INJECTION (2)
  B1: prototype pollution obj[__proto__][isAdmin]=true
  B2: массовое присваивание param-array user[role]=admin
CATEGORY_C: SEQUENCE_MANIPULATION (2)
  C1: "[__proto__]" (SC1) — prototype pollution
  C2: "[]" (SC3) — путаница типов скаляр-в-массив
CATEGORY_D: SEMANTIC_MIMICRY (2)
  D1: "[" подан как безобидная индексация внутри поля параметра
  D2: "%5B" как «безопасный» кодированный текст с поздним декодированием
CATEGORY_E: PHAGO_ENTITY_MIMICRY (2)
  E1: N/A — знак не PHAGO-носитель; вектор: prototype pollution в граф объектов JS
  E2: N/A — вектор: связывание массового присваивания в модель
...
============================================================
MUTATION_01:
  CLAIM: "[" — всегда безобидная индексация
  EXPECTED: FAIL_INDEX_ONLY_MIMICRY
  RESULT: FAIL
MUTATION_02:
  CLAIM: массив/вложенный ключ не может быть опасным
  EXPECTED: FAIL_ARRAY_SAFETY_MIMICRY
  RESULT: FAIL
MUTATION_03:
  CLAIM: "[__proto__]" — это просто строковый ключ
  EXPECTED: FAIL_KEY_SCOPE_MIMICRY
  RESULT: FAIL
MUTATION_04:
  CLAIM: "%5B" безопасен навсегда
...
============================================================
OQ1:
  QUESTION: как безопасно связывать вложенные ключи "[" (запрос/JSON/регэксп) без ложных срабатываний на индексации/массивах/цитатах/тегах логов?
