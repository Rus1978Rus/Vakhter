...
DOCUMENT_ID: SIGN_CORE_CARD_LEFT_TO_RIGHT_ISOLATE_U2066_GEN3_v0_3_EN
...
TRANSLATION_NOTE: English mirror of SIGN_CORE_CARD_LEFT_TO_RIGHT_ISOLATE_U2066_GEN3_v0_3_RU (authoritative). Codepoints, field names and formulas identical. DRAFT for our work (Vakhter); conveyor is a separate project.
...
2. META
============================================================
CARD_UID: SIGN_CORE_CARD_LEFT_TO_RIGHT_ISOLATE_U2066_GEN3_v0_3_EN
CODEPOINT: U+2066
VISIBLE_FORM: ⟨LRI⟩
...
DISPLAY_NAME: left-to-right isolate / scoped bidi reorder
CATEGORY_ROADMAP: LLM (bidi isolate reorder, Trojan Source) · PHAGO: — (structure masking)
GLYPH_NOTE: VISIBLE_FORM uses the marker ⟨LRI⟩; the sign itself (U+2066) is an invisible Bidi_Control (Cf) and is NEVER written literally here — a literal LRI would reorder this document. Examples use ⟨LRI⟩/⟨PDI⟩/%E2%81%A6, never the byte.
...
SIGN_CATEGORY:
  - bidirectional isolate (opens an LTR run isolated from its surroundings)
  - Unicode Bidi_Control, the modern recommended alternative to embeddings/overrides
  - legitimate scoped mixed-direction layout (does not affect neighbours)
  - (misused) newer control a legacy embedding/override filter does not model
...
WHAT_THIS_SIGN_IS_NOT:
  ...
  5. NOT_ENCODED_SAFE — "%E2%81%A6" may be decoded back to the isolate later
...
NOTE: LRI (ZONE_1) has parallel functions (legitimate scoped LTR layout vs. in-scope visual-order deception) co-existing without cultural precession. Polysemy of a stable Bidi_Control.
LAYER_ANOMALY:
...
RISK_CASES:
  ...
  RISK_CASE_005:
    ...
    INPUT: "value%E2%81%A6tail (with a later decode)"
    CONTEXT: a percent-encoded LRI decoded back before display
    RISK: HIGH
    ATTACK: "%E2%81%A6" decodes to the isolate AFTER a check → reorder deception
    GUARD: LRI_FORM ≠ ENCODED_SAFETY_PROOF
  RISK_CASE_006:
    NAME: BIDI_HOMOGLYPH_STACK
    INPUT: "раyраl<LRI> ... (bidi + confusable letters combined)"
...
CONFUSABLES:
  CONFUSABLE_001:
    ...
    RISK: HIGH
    RULE: RIGHT_TO_LEFT_ISOLATE ≠ LEFT_TO_RIGHT_ISOLATE (mirror isolate, opposite direction; a naive filter conflates them)
  CONFUSABLE_002:
    VISIBLE_FORM: ⟨FSI⟩
    CODEPOINT: U+2068
    NAME: FIRST STRONG ISOLATE
    RISK: HIGH
    RULE: FIRST_STRONG_ISOLATE ≠ LEFT_TO_RIGHT_ISOLATE (FSI auto-picks direction from the first strong char — attacker-controllable)
  CONFUSABLE_003:
    VISIBLE_FORM: ⟨LRE⟩
    CODEPOINT: U+202A
    NAME: LEFT-TO-RIGHT EMBEDDING
    RISK: MEDIUM
    RULE: LEFT_TO_RIGHT_EMBEDDING ≠ LEFT_TO_RIGHT_ISOLATE (embedding affects neighbours; isolate scopes — different model)
  CONFUSABLE_004:
    VISIBLE_FORM: ⟨PDI⟩
    CODEPOINT: U+2069
    NAME: POP DIRECTIONAL ISOLATE
    RISK: LOW
    RULE: POP_DIRECTIONAL_ISOLATE ≠ LEFT_TO_RIGHT_ISOLATE (the terminator, not the opener)
  CONFUSABLE_005:
    VISIBLE_FORM: ⟨RLO⟩
    CODEPOINT: U+202E
    NAME: RIGHT-TO-LEFT OVERRIDE
    RISK: LOW
    RULE: RIGHT_TO_LEFT_OVERRIDE ≠ LEFT_TO_RIGHT_ISOLATE (override forces & affects neighbours; isolate scopes)
...
CONTRADICTION_GUARDS:
  ...
  CG2:
    ...
    RULE: invisibility is orthogonal to effect; LRI drives visual/logical desync
  CG3:
  ...
  CG4:
    TRIGGER: "'%E2%81%A6' is safe forever"
...
SEQUENCE_LAYER_BOUNDARY:
  ...
  SEQUENCE_CANDIDATES:
    ...
    SC3:
      ...
      REQUIRES_SEQUENCE_INTEGRATOR: YES
  IF NOT_APPLICABLE: not applicable — sequences with LRI are central to scoped visual-order deception.

PHAGO_ENTITY_MIMICRY:
  NOT_APPLICABLE:
    REASON: LRI reorders a scoped run (structure masking), but does not imitate the existence of a verified entity. Its risks are visual/logical desync, not entity mimicry. (Filename spoof is more natural with override; see RLO/LRO.)
    REVIEW_REQUIRED: YES
...
CATEGORY_A: FORM_MANIPULATION (2)
  A1: substitution of LRI with RLI (U+2067) / FSI (U+2068) to vary direction / evade an LRI-only filter
  A2: percent-encoding "%E2%81%A6" to slip past a raw-byte scan
CATEGORY_B: CONTEXT_INJECTION (2)
  B1: legacy filter gap (isolate survives a 202A-202E-only strip)
  B2: in-scope code reorder safe=true <LRI> // danger? <PDI>
CATEGORY_C: SEQUENCE_MANIPULATION (2)
  C1: "LRI (no PDI)" (SC2) — unterminated bleed to paragraph end
  C2: "LRI ... PDF" (SC3) — wrong terminator
CATEGORY_D: SEMANTIC_MIMICRY (2)
  D1: LRI presented as a harmless "recommended safe" isolate inside a code field
  D2: "%E2%81%A6" as "safe" encoded text with a later decode
...
MUTATION_03:
  ...
  RESULT: FAIL
MUTATION_04:
  CLAIM: "%E2%81%A6" is safe forever
...
============================================================
OQ1:
  QUESTION: how to model the full Bidi_Control set incl. isolates (2066-2069) and their FSI auto-direction, without false positives on legitimate scoped mixed-direction text?
