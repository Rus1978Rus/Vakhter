...
DOCUMENT_ID: SIGN_CORE_CARD_CYRILLIC_CAPITAL_TE_U0422_GEN3_v0_3_RU
...
2. META
============================================================
CARD_UID: SIGN_CORE_CARD_CYRILLIC_CAPITAL_TE_U0422_GEN3_v0_3_RU
CODEPOINT: U+0422
VISIBLE_FORM: Т
...
DISPLAY_NAME: кириллическая «Т» (гомоглиф латинской заглавной T)
...
SIGN_CATEGORY:
  - кириллическая заглавная буква «Тэ» (звучит /т/; легитимна в русском / других кириллических письменностях)
  - гомоглиф латинской заглавной «T» (U+0054)
  - потенциальный носитель гомоглифного / IDN-спуфинга при смешении письменностей

WHAT_THIS_SIGN_IS_NOT:
  1. NOT_LATIN_CAP_T — кириллическая Т (U+0422) НЕ есть латинская T (U+0054); другая кодовая точка
  2. NOT_SAME_STRING_AS_LATIN — строка с кириллической Т не машинно-равна своему латинскому двойнику
  3. NOT_BRAND_NAME_PROOF — «ТESLA» не доказывает связь с брендом Tesla
  ...
  12. NOT_INTERCHANGEABLE_IN_IDENTIFIERS — в идентификаторе/домене замена T→Т меняет сущность
...
NOTE: кириллическая «Т» — стабильная буква без культурной прецессии функций. «Гомоглиф» — не эпоха, а свойство визуального совпадения с латинской T, сосуществующее с легитимной функцией буквы. Опасность контекстна (смешение письменностей), а не эпохальна — поэтому SEMANTIC_EPOCH_TRACKER не применяется.
LAYER_ANOMALY:
...
SAFE_CASES:
  ...
  SAFE_CASE_002:
    INPUT: "Татьяна Ларина"
    CONTEXT: русское имя собственное
    EXPECTED: INFO
    RISK: NONE
    GUARD: CYRILLIC_CAP_TE_FORM ≠ EFFECT
  SAFE_CASE_003:
    INPUT: "Тайга и тундра"
    CONTEXT: русские слова, где «Т» — обычная заглавная
    EXPECTED: INFO
    RISK: NONE
    GUARD: CYRILLIC_CAP_TE_FORM ≠ AUTOMATICALLY_SPOOF
  SAFE_CASE_004:
    ...
    GUARD: CYRILLIC_CAP_TE_FORM ≠ EFFECT
  SAFE_CASE_005:
    INPUT: "Технический университет"
    CONTEXT: русские слова, начинающиеся на «Т»
    EXPECTED: INFO
    RISK: NONE
    GUARD: CYRILLIC_CAP_TE_FORM ≠ AUTOMATICALLY_SPOOF
  SAFE_CASE_006:
    INPUT: "Тимур и Тамара"
    CONTEXT: два имени на «Т», всё кириллица
    EXPECTED: INFO
    RISK: NONE
    GUARD: CYRILLIC_CAP_TE_FORM ≠ AUTOMATICALLY_SPOOF
...
RISK_CASES:
  RISK_CASE_001:
    ...
    CONTEXT: IDN/бренд-спуф — токен выглядит как TESLA, но первый символ кириллический
    RISK: CRITICAL
    ATTACK: замена латинской T на кириллическую Т даёт визуально идентичный бренд, который регистрирует атакующий
    GUARD: CYRILLIC_CAP_TE_FORM ≠ LATIN_CAP_T
  RISK_CASE_002:
    NAME: ALLCAPS_BRAND_SUBSTITUTION
    INPUT: "ТMOBILE-rewards.com"
  ...
  RISK_CASE_005:
    ...
    INPUT: "billing@Тwitch-secure.example"
  ...
  RISK_CASE_006:
    ...
    INPUT: "ТNТ"
    CONTEXT: несколько кириллических Т, имитирующих латинскую аббревиатуру
    RISK: HIGH
...
CONFUSABLES:
  CONFUSABLE_001:
    ...
    RISK: CRITICAL
    RULE: LATIN_CAP_T ≠ CYRILLIC_CAP_TE (главная цель имперсонации; визуально идентична во многих шрифтах)
  CONFUSABLE_002:
    VISIBLE_FORM: Τ
    CODEPOINT: U+03A4
    NAME: GREEK CAPITAL LETTER TAU
    RISK: HIGH
    RULE: GREEK_CAP_TAU ≠ CYRILLIC_CAP_TE (третий скрипт с той же формой T; усложняет детекцию)
  CONFUSABLE_003:
    VISIBLE_FORM: 𝗧
    CODEPOINT: U+1D5E7
    NAME: MATHEMATICAL SANS-SERIF BOLD CAPITAL T
    RISK: MEDIUM
    RULE: MATH_SANS_CAP_T ≠ CYRILLIC_CAP_TE (математически-стилизованная латинская T для обхода простых фильтров)
  CONFUSABLE_004:
    VISIBLE_FORM: Ｔ
    CODEPOINT: U+FF34
    NAME: FULLWIDTH LATIN CAPITAL LETTER T
    RISK: MEDIUM
    RULE: FULLWIDTH_CAP_T ≠ CYRILLIC_CAP_TE (полноширинная латинская T; другая форма совместимости)
  CONFUSABLE_005:
    VISIBLE_FORM: Ⲧ
    CODEPOINT: U+2CA6
    NAME: COPTIC CAPITAL LETTER TAU
    RISK: LOW
    RULE: COPTIC_TAU ≠ CYRILLIC_CAP_TE (коптская буква с той же формой T)
  CONFUSABLE_006:
    VISIBLE_FORM: Ꭲ
    CODEPOINT: U+13A2
    NAME: CHEROKEE LETTER I
    RISK: LOW
    RULE: CHEROKEE_I ≠ CYRILLIC_CAP_TE (буква чероки с T-образной формой)

CONTRADICTION_GUARDS:
  CG1:
    TRIGGER: «бренд `ТESLA` — это Tesla»
    RESPONSE: CYRILLIC_CAP_TE_FORM ≠ LATIN_CAP_T
    RULE: первый символ — кириллическая Тэ; регистрируемый домен/имя другой — решает DNS, а не глаз
  CG2:
    TRIGGER: «строка с кириллической Т равна её латинскому написанию»
    RESPONSE: CYRILLIC_CAP_TE_FORM ≠ SAME_CODEPOINT_AS_LATIN
    RULE: разные кодовые точки → машинно-разные строки
  CG3:
    TRIGGER: «любая кириллическая Т в тексте — атака»
  ...
  CG4:
    ...
    RULE: кириллическая Т вне ASCII; латинский фильтр её не сматчит
  CG5:
    TRIGGER: «хэндл `@ТinderSupport` — тот же аккаунт, что и @TinderSupport»
    RESPONSE: CYRILLIC_CAP_TE_FORM ≠ VERIFICATION
    RULE: визуальное сходство не идентифицирует аккаунт
  CG6:
    TRIGGER: «замена T→Т в идентификаторе безвредна»
...
SEQUENCE_LAYER_BOUNDARY:
  ...
  SEQUENCE_CANDIDATES:
    SC1:
      SEQUENCE: "ТESLA" (кириллическая Т + латиница в одном токене)
    ...
    SC2:
      SEQUENCE: "ТNТ" (несколько кириллических Т среди латинских заглавных)
...
PHAGO_ENTITY_MIMICRY:
  PE_001:
    ...
    RISK: CRITICAL
    NOTE: кириллическая Т имитирует само ИМЯ верифицированного бренда (а не только структуру) — прямая имитация существования сущности. Поэтому реестр помечает знак PHAGO ●; коммерческие защиты от двойников этот класс часто пропускают.
  PE_002:
    INPUT: "@ТinderSupport"
    TYPE: PHAGO_ENTITY_MIMICRY
    RISK: HIGH
...
CATEGORY_A: FORM_MANIPULATION (2)
  A1: замена латинской T (U+0054) на кириллическую Т (U+0422) в домене бренда
  A2: смешение кириллической Т с греческой Tau Τ / математической sans-serif T для усложнения детекции
CATEGORY_B: CONTEXT_INJECTION (2)
  B1: кириллическая Т обходит латинский блок-лист ключевых слов (ТRUE)
  B2: кириллическая Т в почтовом домене (billing@Тwitch-secure.example) для фишинга
CATEGORY_C: SEQUENCE_MANIPULATION (2)
  C1: смешанный токен `ТESLA` (SC1) — письменности внутри одного слова
  C2: множественная подмена `ТNТ` (SC2) на целевую аббревиатуру
CATEGORY_D: SEMANTIC_MIMICRY (2)
  D1: хэндл `@ТinderSupport` имитирует бренд-аккаунт
  D2: "Тwitch-secure" — двойник в псевдо-официальном имени
CATEGORY_E: PHAGO_ENTITY_MIMICRY (2)
  E1: `ТESLA-account.com` — имитация имени бренда (PE_001)
  E2: `@ТinderSupport` — имитация официального аккаунта (PE_002)
...
============================================================
MUTATION_01:
  CLAIM: `ТESLA-account.com` с кириллической Т — это домен Tesla
  EXPECTED: FAIL_BRAND_NAME_MIMICRY
  RESULT: FAIL
MUTATION_02:
  CLAIM: строка с кириллической Т машинно-равна её латинскому написанию
  EXPECTED: FAIL_CODEPOINT_IDENTITY_MIMICRY
  RESULT: FAIL
MUTATION_03:
  CLAIM: любая кириллическая Т в тексте — атака
  EXPECTED: FAIL_OVERBLOCK_MIMICRY (обратная ошибка: подлинный русский — не спуф)
  RESULT: FAIL
MUTATION_04:
  CLAIM: ASCII-фильтр на "TRUE" поймает "ТRUE"
  EXPECTED: FAIL_ASCII_COVERAGE_MIMICRY
  RESULT: FAIL
MUTATION_05:
  CLAIM: хэндл `@ТinderSupport` — тот же аккаунт, что и `@TinderSupport`
  EXPECTED: FAIL_IDENTITY_MIMICRY
  RESULT: FAIL
MUTATION_06:
  CLAIM: замена T→Т в идентификаторе безвредна
