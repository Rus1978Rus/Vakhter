...
DOCUMENT_ID: SIGN_CORE_CARD_CYRILLIC_ES_U0441_GEN3_v0_3_EN
...
TRANSLATION_NOTE: English mirror of SIGN_CORE_CARD_CYRILLIC_ES_U0441_GEN3_v0_3_RU (authoritative). Codepoints, field names and formulas identical. Homoglyph sign: core law is LOOKS_SAME ≠ IS_SAME. DRAFT for our work (Vakhter); conveyor is a separate project.
...
2. META
============================================================
CARD_UID: SIGN_CORE_CARD_CYRILLIC_ES_U0441_GEN3_v0_3_EN
CODEPOINT: U+0441
VISIBLE_FORM: с
...
DISPLAY_NAME: Cyrillic "с" (homoglyph of Latin c)
...
WHAT_THIS_SIGN_IS_NOT:
  1. NOT_LATIN_C — Cyrillic с (U+0441) is NOT Latin c (U+0063); a different codepoint
  2. NOT_SAME_STRING_AS_LATIN — a string with Cyrillic с is not machine-equal to its Latin twin
  3. NOT_BRAND_NAME_PROOF — "сisco.com" does not prove a link to the Cisco brand
...
NOTE: Cyrillic "с" is a stable letter with no cultural precession of functions. "Homoglyph" is not an epoch but a property of visual coincidence with Latin c, co-existing with the letter's legit function. The danger is contextual (mixed scripts), not epochal — so SEMANTIC_EPOCH_TRACKER does not apply.
LAYER_ANOMALY:
...
SAFE_CASES:
  ...
  SAFE_CASE_003:
    ...
    CONTEXT: Russian words where "с" is an ordinary letter
    EXPECTED: INFO
    RISK: NONE
    GUARD: CYRILLIC_ES_FORM ≠ AUTOMATICALLY_SPOOF
  SAFE_CASE_004:
  ...
  SAFE_CASE_006:
    ...
    CONTEXT: a phrase with "с", yet all Cyrillic
    EXPECTED: INFO
    RISK: NONE
    GUARD: CYRILLIC_ES_FORM ≠ AUTOMATICALLY_SPOOF
...
RISK_CASES:
  RISK_CASE_001:
    ...
    CONTEXT: IDN/brand spoof — the token looks like cisco.com but the first char is Cyrillic
    RISK: CRITICAL
    ATTACK: replacing Latin c with Cyrillic с yields a visually identical domain the attacker registers
  ...
  RISK_CASE_002:
    ...
    RISK: CRITICAL
    ATTACK: the string is machine-≠ icloud.com, but a human sees no difference
    GUARD: CYRILLIC_ES_FORM ≠ DOMAIN_VALIDITY_PROOF
  RISK_CASE_003:
    NAME: HANDLE_IMPERSONATION
    INPUT: "@сustomer_care" (Cyrillic с in a handle)
    CONTEXT: impersonating a customer-care account in a chat/social platform
    RISK: HIGH
    ATTACK: the look-alike handle appears as customer_care but is a different account
  ...
  RISK_CASE_005:
    ...
    CONTEXT: a phishing email from the "same" cloud provider
    RISK: HIGH
...
CONFUSABLES:
  CONFUSABLE_001:
    ...
    RISK: CRITICAL
    RULE: LATIN_C ≠ CYRILLIC_ES (the primary impersonation target; visually identical in many fonts)
  CONFUSABLE_002:
    VISIBLE_FORM: ϲ
    CODEPOINT: U+03F2
    NAME: GREEK LUNATE SIGMA SYMBOL
    RISK: HIGH
    RULE: GREEK_LUNATE_SIGMA ≠ CYRILLIC_ES (a third script with the same c-like shape; complicates detection)
  CONFUSABLE_003:
    VISIBLE_FORM: ⲥ
    CODEPOINT: U+2CA5
    NAME: COPTIC SMALL LETTER SIMA
    RISK: MEDIUM
    RULE: COPTIC_SIMA ≠ CYRILLIC_ES (another look-alike from a fourth script)
  CONFUSABLE_004:
    VISIBLE_FORM: 𝖼
    CODEPOINT: U+1D5BC
    NAME: MATHEMATICAL SANS-SERIF SMALL C
    RISK: MEDIUM
    RULE: MATH_SANS_C ≠ CYRILLIC_ES (a math-alphanumeric styled Latin c used to dodge simple filters)
  CONFUSABLE_005:
    VISIBLE_FORM: ｃ
    CODEPOINT: U+FF43
    NAME: FULLWIDTH LATIN SMALL LETTER C
    RISK: LOW
    RULE: FULLWIDTH_C ≠ CYRILLIC_ES (a full-width Latin c; a different compatibility form)
  CONFUSABLE_006:
    VISIBLE_FORM: ç
    CODEPOINT: U+00E7
    NAME: LATIN SMALL LETTER C WITH CEDILLA
    RISK: LOW
    RULE: LATIN_C_CEDILLA ≠ CYRILLIC_ES (an accented Latin twin)
...
CATEGORY_A: FORM_MANIPULATION (2)
  A1: substitution of Latin c (U+0063) with Cyrillic с (U+0441) in a brand domain
  A2: mixing Cyrillic с with Greek lunate sigma ϲ / math sans-serif c to complicate detection
CATEGORY_B: CONTEXT_INJECTION (2)
  B1: Cyrillic с bypasses a Latin-keyword blocklist (сasino)
  B2: Cyrillic с in a mail domain (no-reply@сloud-billing.example) for phishing
CATEGORY_C: SEQUENCE_MANIPULATION (2)
  C1: the mixed token `сisco` (SC1) — scripts within one word
  C2: multiple substitution `сoinbaсe` (SC2) onto a target brand
CATEGORY_D: SEMANTIC_MIMICRY (2)
  D1: the handle `@сustomer_care` imitates a service account
  D2: "сloud-official" — a look-alike in a pseudo-official name
CATEGORY_E: PHAGO_ENTITY_MIMICRY (2)
  E1: `сisco.com` — mimicry of a brand name (PE_001)
  E2: `@iсloud_support` — mimicry of an official account (PE_002)
