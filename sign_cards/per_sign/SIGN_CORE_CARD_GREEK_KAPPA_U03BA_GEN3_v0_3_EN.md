...
DOCUMENT_ID: SIGN_CORE_CARD_GREEK_KAPPA_U03BA_GEN3_v0_3_EN
...
TRANSLATION_NOTE: English mirror of SIGN_CORE_CARD_GREEK_KAPPA_U03BA_GEN3_v0_3_RU (authoritative). Codepoints, field names and formulas identical. Homoglyph sign: core law is LOOKS_SAME ≠ IS_SAME. DRAFT for our work (Vakhter); conveyor is a separate project.
...
2. META
============================================================
CARD_UID: SIGN_CORE_CARD_GREEK_KAPPA_U03BA_GEN3_v0_3_EN
CODEPOINT: U+03BA
VISIBLE_FORM: κ
...
DISPLAY_NAME: Greek "κ" kappa (homoglyph of Latin small k; reads /k/)
...
SIGN_CATEGORY:
  - Greek small letter kappa (sounds /k/; legitimate in Greek script)
  - homoglyph of Latin small "k" (U+006B)
  - potential carrier of homoglyph / IDN spoofing when scripts are mixed

WHAT_THIS_SIGN_IS_NOT:
  1. NOT_LATIN_SMALL_K — Greek κ (U+03BA) is NOT Latin k (U+006B); a different codepoint in a different script
  2. NOT_SAME_STRING_AS_LATIN — a string with Greek κ is not machine-equal to its Latin twin
  3. NOT_BRAND_NAME_PROOF — "κaspersky" does not prove a link to the Kaspersky brand
...
NOTE: Greek "κ" kappa is a stable letter with no cultural precession of functions. "Homoglyph" is not an epoch but a property of visual coincidence with Latin k, co-existing with the letter's legit function. The danger is contextual (mixed scripts), not epochal — so SEMANTIC_EPOCH_TRACKER does not apply.
LAYER_ANOMALY:
...
SAFE_CASES:
  SAFE_CASE_001:
    INPUT: "καλημέρα κόσμε" (good morning world, Greek)
  ...
  SAFE_CASE_002:
    ...
    CONTEXT: a Greek phrase where "κ" is an ordinary letter
    EXPECTED: INFO
    RISK: NONE
  ...
  SAFE_CASE_003:
    ...
    GUARD: GREEK_KAPPA_FORM ≠ AUTOMATICALLY_SPOOF
  SAFE_CASE_004:
    INPUT: "κύκλος και γραμμή" (circle and line, Greek)
    CONTEXT: single-script Greek phrase using "κ"
    EXPECTED: INFO
    RISK: NONE
    GUARD: GREEK_KAPPA_FORM ≠ EFFECT
  SAFE_CASE_005:
    INPUT: "κιθάρα και τραγούδι" (guitar and song, Greek)
    CONTEXT: Greek words using "κ"
    EXPECTED: INFO
    RISK: NONE
    GUARD: GREEK_KAPPA_FORM ≠ AUTOMATICALLY_SPOOF
  SAFE_CASE_006:
    INPUT: "καλό και κακό" (good and evil, Greek)
    CONTEXT: single-script Greek phrase
    EXPECTED: INFO
    RISK: NONE
    GUARD: GREEK_KAPPA_FORM ≠ AUTOMATICALLY_SPOOF
...
RISK_CASES:
  RISK_CASE_001:
    ...
    INPUT: "κaspersky-login.com" (Greek κ at the start of a brand)
    CONTEXT: IDN/brand spoof — the token looks like kaspersky but the first char is Greek
    RISK: CRITICAL
  ...
  RISK_CASE_002:
    ...
    INPUT: "κraken-exchange.com" (Greek κ in a brand)
    CONTEXT: leading substitution in a brand phrase
    RISK: CRITICAL
    ATTACK: the string is machine-≠ kraken, but a human sees no difference
  ...
  RISK_CASE_004:
    ...
    INPUT: "κill" (Greek κ bypasses a "kill" blocklist)
  ...
  RISK_CASE_005:
    ...
    INPUT: "no-reply@κohls-deals.example" (Greek κ in the mail domain)
  ...
  RISK_CASE_006:
    ...
    INPUT: "κiκ" (two Greek κ around a Latin i, imitating kik)
    CONTEXT: multiple substitutions in one token (κ Greek, i Latin — a script mix)
    RISK: HIGH
...
CONFUSABLES:
  CONFUSABLE_001:
    ...
    RISK: CRITICAL
    RULE: LATIN_SMALL_K ≠ GREEK_KAPPA (the primary impersonation target; visually similar in many fonts)
  CONFUSABLE_002:
    VISIBLE_FORM: к
    CODEPOINT: U+043A
    NAME: CYRILLIC SMALL LETTER KA
    RISK: HIGH
    RULE: CYRILLIC_KA ≠ GREEK_KAPPA (a third script with the same k-shape; complicates detection)
  CONFUSABLE_003:
    VISIBLE_FORM: ｋ
    CODEPOINT: U+FF4B
    NAME: FULLWIDTH LATIN SMALL LETTER K
    RISK: MEDIUM
    RULE: FULLWIDTH_SMALL_K ≠ GREEK_KAPPA (a full-width Latin k; a different compatibility form)
  CONFUSABLE_004:
    VISIBLE_FORM: 𝑘
    CODEPOINT: U+1D458
    NAME: MATHEMATICAL ITALIC SMALL K
    RISK: MEDIUM
    RULE: MATH_ITALIC_SMALL_K ≠ GREEK_KAPPA (a math-alphanumeric styled Latin k used to dodge simple filters)
  CONFUSABLE_005:
    VISIBLE_FORM: 𝗄
    CODEPOINT: U+1D5C4
    NAME: MATHEMATICAL SANS-SERIF SMALL K
    RISK: LOW
    RULE: MATH_SANS_SMALL_K ≠ GREEK_KAPPA (a sans-serif styled Latin k)
  CONFUSABLE_006:
    VISIBLE_FORM: ⲕ
    CODEPOINT: U+2C95
    NAME: COPTIC SMALL LETTER KAPA
    RISK: LOW
    RULE: COPTIC_KAPA ≠ GREEK_KAPPA (a Coptic letter with the same k-shape)

CONTRADICTION_GUARDS:
  CG1:
    TRIGGER: "the brand `κaspersky` is Kaspersky"
    RESPONSE: GREEK_KAPPA_FORM ≠ LATIN_SMALL_K
    RULE: the first char is Greek kappa; the registrable domain/name is different — DNS decides, not the eye
  CG2:
...
CATEGORY_A: FORM_MANIPULATION (2)
  A1: substitution of Latin k (U+006B) with Greek κ (U+03BA) in a brand domain
  A2: mixing Greek κ with Cyrillic ka к / math-styled k to complicate detection
CATEGORY_B: CONTEXT_INJECTION (2)
  B1: Greek κ bypasses a Latin-keyword blocklist (κill)
  B2: Greek κ in a mail domain (no-reply@κohls-deals.example) for phishing
CATEGORY_C: SEQUENCE_MANIPULATION (2)
  C1: the mixed token `κaspersky` (SC1) — scripts within one word
  C2: multiple substitution `κiκ` (SC2) onto a target word
CATEGORY_D: SEMANTIC_MIMICRY (2)
  D1: the handle `@κrakenHelp` imitates a brand account
  D2: "κohls-deals" — a look-alike in a pseudo-official name
CATEGORY_E: PHAGO_ENTITY_MIMICRY (2)
  E1: `κaspersky-login.com` — mimicry of a brand name (PE_001)
...
============================================================
MUTATION_01:
  CLAIM: `κaspersky-login.com` with Greek κ is Kaspersky's domain
...
MUTATION_03:
  ...
  RESULT: FAIL
MUTATION_04:
  CLAIM: an ASCII filter on "kill" will catch "κill"
