...
DOCUMENT_ID: SIGN_CORE_CARD_ARMENIAN_SE_U057D_GEN3_v0_3_EN
...
TRANSLATION_NOTE: English mirror of SIGN_CORE_CARD_ARMENIAN_SE_U057D_GEN3_v0_3_RU (authoritative). Codepoints, field names and formulas identical. Homoglyph sign: core law is LOOKS_SAME ≠ IS_SAME. DRAFT for our work (Vakhter); conveyor is a separate project.
...
2. META
============================================================
CARD_UID: SIGN_CORE_CARD_ARMENIAN_SE_U057D_GEN3_v0_3_EN
CODEPOINT: U+057D
VISIBLE_FORM: ս
...
DISPLAY_NAME: Armenian "ս" seh (homoglyph of Latin small u)
...
SIGN_CATEGORY:
  - Armenian seh — a consonant letter of the Armenian alphabet (sounds /s/)
  - homoglyph of Latin small "u" (U+0075)
  - potential carrier of homoglyph / IDN spoofing when scripts are mixed

WHAT_THIS_SIGN_IS_NOT:
  1. NOT_LATIN_SMALL_U — Armenian ս (U+057D) is NOT Latin u (U+0075); a different codepoint and the consonant /s/
  ...
  12. NOT_INTERCHANGEABLE_IN_IDENTIFIERS — in an identifier/domain, u→ս changes the entity
...
NOTE: Armenian the letter seh is a stable letter with no cultural precession of functions. "Homoglyph" is not an epoch but a property of visual coincidence with Latin u, co-existing with the letter's legit function. The danger is contextual (mixed scripts), not epochal — so SEMANTIC_EPOCH_TRACKER does not apply.
LAYER_ANOMALY:
  ABSENT_PHYSICAL_LAYER: YES, level L1 (physical gesture)
  NOTE: a letter is a written sign with no gestural predecessor; the letter seh is a written glyph of the Armenian script.
...
SAFE_CASES:
  SAFE_CASE_001:
    INPUT: "սեր" (Armenian (love))
    CONTEXT: single-script native text
    EXPECTED: INFO
    RISK: NONE
    GUARD: ARMENIAN_SE_FORM ≠ AUTOMATICALLY_SPOOF
  SAFE_CASE_002:
    INPUT: "սիրտ" (Armenian (a heart))
    CONTEXT: single-script native text
    EXPECTED: INFO
    RISK: NONE
    GUARD: ARMENIAN_SE_FORM ≠ EFFECT
  SAFE_CASE_003:
    INPUT: "սար" (Armenian (a mountain))
    CONTEXT: single-script native text
    EXPECTED: INFO
    RISK: NONE
    GUARD: ARMENIAN_SE_FORM ≠ AUTOMATICALLY_SPOOF
  SAFE_CASE_004:
    INPUT: "սուրբ" (Armenian (holy))
    CONTEXT: single-script native text
    EXPECTED: INFO
    RISK: NONE
    GUARD: ARMENIAN_SE_FORM ≠ EFFECT
  SAFE_CASE_005:
    INPUT: "սեղան" (Armenian (a table))
    CONTEXT: single-script native text
    EXPECTED: INFO
    RISK: NONE
    GUARD: ARMENIAN_SE_FORM ≠ AUTOMATICALLY_SPOOF
  SAFE_CASE_006:
    INPUT: "սառույց" (Armenian (ice))
    CONTEXT: single-script native text
    EXPECTED: INFO
    RISK: NONE
    GUARD: ARMENIAN_SE_FORM ≠ AUTOMATICALLY_SPOOF
...
RISK_CASES:
  RISK_CASE_001:
    ...
    INPUT: "սber-login.com" (IDN/brand spoof — the token looks like uber but a letter is Armenian)
    CONTEXT: IDN/brand spoof — the token looks like uber but a letter is Armenian
    RISK: CRITICAL
    ATTACK: replacing Latin u with Armenian ս yields a visually identical brand the attacker registers
    GUARD: ARMENIAN_SE_FORM ≠ LATIN_SMALL_U
  RISK_CASE_002:
    NAME: BRAND_DOMAIN_SUBSTITUTION
    INPUT: "սbuntu" (a look-alike inside the token `ubuntu`)
    CONTEXT: a look-alike inside the token `ubuntu`
    RISK: CRITICAL
    ATTACK: the string is machine-≠ ubuntu, but a human sees no difference
    GUARD: ARMENIAN_SE_FORM ≠ DOMAIN_VALIDITY_PROOF
  RISK_CASE_003:
    NAME: HANDLE_IMPERSONATION
    INPUT: "@սberSupport" (impersonating a brand account in a chat/social platform)
  ...
  RISK_CASE_004:
    ...
    INPUT: "accoսnt" (bypassing a text filter looking for the Latin word `account`)
    CONTEXT: bypassing a text filter looking for the Latin word `account`
    RISK: HIGH
    ATTACK: one substituted letter moves the word out from under the blocklist
    GUARD: ARMENIAN_SE_FORM ≠ ASCII_LETTER
  RISK_CASE_005:
    NAME: EMAIL_LOOKALIKE
    INPUT: "billing@սsbank-secure.example" (a phishing email from the "same" company)
  ...
  RISK_CASE_006:
    ...
    INPUT: "yoսtube-billing.com" (another target brand (youtube) opened with the look-alike)
    CONTEXT: another target brand (youtube) opened with the look-alike
    RISK: HIGH
...
CONFUSABLES:
  CONFUSABLE_001:
    ...
    RISK: CRITICAL
    RULE: LATIN_SMALL_LETTER_U ≠ ARMENIAN_SE_FORM (the primary impersonation target; visually identical in many fonts)
  CONFUSABLE_002:
    VISIBLE_FORM: ｕ
    CODEPOINT: U+FF55
    NAME: FULLWIDTH LATIN SMALL LETTER U
    RISK: MEDIUM
    RULE: FULLWIDTH_LATIN_SMALL_LETTER_U ≠ ARMENIAN_SE_FORM (a full-width Latin form; a different compatibility form)
  CONFUSABLE_003:
    VISIBLE_FORM: 𝗎
    CODEPOINT: U+1D5CE
    NAME: MATHEMATICAL SANS-SERIF SMALL U
    RISK: MEDIUM
    RULE: MATHEMATICAL_SANS_SERIF_SMALL_U ≠ ARMENIAN_SE_FORM (a math-alphanumeric styled Latin letter used to dodge simple filters)
  CONFUSABLE_004:
    VISIBLE_FORM: 𝐮
    CODEPOINT: U+1D42E
    NAME: MATHEMATICAL BOLD SMALL U
    RISK: LOW
    RULE: MATHEMATICAL_BOLD_SMALL_U ≠ ARMENIAN_SE_FORM (a bold-styled Latin letter)
  CONFUSABLE_005:
    VISIBLE_FORM: 𝑢
    CODEPOINT: U+1D462
    NAME: MATHEMATICAL ITALIC SMALL U
    RISK: LOW
    RULE: MATHEMATICAL_ITALIC_SMALL_U ≠ ARMENIAN_SE_FORM (an italic-styled Latin letter)
  CONFUSABLE_006:
    VISIBLE_FORM: 𝚞
    CODEPOINT: U+1D69E
    NAME: MATHEMATICAL MONOSPACE SMALL U
    RISK: LOW
    RULE: MATHEMATICAL_MONOSPACE_SMALL_U ≠ ARMENIAN_SE_FORM (a monospace-styled Latin letter)

CONTRADICTION_GUARDS:
  CG1:
    TRIGGER: "the brand `սber` is Uber"
    RESPONSE: ARMENIAN_SE_FORM ≠ LATIN_SMALL_U
    RULE: the letter is Armenian ս (and the consonant /s/); the registrable domain/name is different — DNS decides, not the eye
  CG2:
  ...
  CG5:
    TRIGGER: "the handle `@սberSupport` is the same account as the Latin one"
    RESPONSE: ARMENIAN_SE_FORM ≠ VERIFICATION
    RULE: visual similarity does not identify an account
  CG6:
    TRIGGER: "swapping u→ս in an identifier is harmless"
...
SEQUENCE_LAYER_BOUNDARY:
  ...
  SEQUENCE_CANDIDATES:
    SC1:
      SEQUENCE: "սber-login.com" (Armenian ս + Latin in one token)
      NAME: MIXED_SCRIPT_TOKEN
      RISK_LEVEL: CRITICAL
      POSSIBLE_CONTEXTS: IDN/brand spoof; the key signal is a SCRIPT MIX within one token
      REQUIRES_SEQUENCE_INTEGRATOR: YES
    SC2:
      SEQUENCE: "yoսtube-billing.com" (capital-initial look-alike on a target brand)
...
CATEGORY_A: FORM_MANIPULATION (2)
  A1: substitution of Latin u (U+0075) with Armenian ս (U+057D) in a brand domain
  A2: mixing ս with math/full-width Latin u to complicate detection
CATEGORY_B: CONTEXT_INJECTION (2)
  B1: ս bypasses a Latin-keyword blocklist (accoսnt)
  B2: ս in a mail domain (billing@սsbank-secure.example) for phishing
CATEGORY_C: SEQUENCE_MANIPULATION (2)
  C1: the mixed token `սber-login.com` (SC1) — scripts within one word
  C2: opening/multiple substitution `yoսtube-billing.com` (SC2) onto a target brand
CATEGORY_D: SEMANTIC_MIMICRY (2)
  D1: the handle `@սberSupport` imitates a brand account
  D2: `billing@սsbank-secure.example` — a look-alike in a pseudo-official name
CATEGORY_E: PHAGO_ENTITY_MIMICRY (2)
  E1: `սber-login.com` — mimicry of a brand name (PE_001)
  E2: `@սberSupport` — mimicry of an official account (PE_002)
...
============================================================
MUTATION_01:
  CLAIM: `սber-login.com` with ս is the real brand's domain
...
MUTATION_03:
  ...
  RESULT: FAIL
MUTATION_04:
  CLAIM: an ASCII filter on the Latin word will catch `accoսnt`
  EXPECTED: FAIL_ASCII_COVERAGE_MIMICRY
  RESULT: FAIL
MUTATION_05:
  CLAIM: the handle `@սberSupport` is the same account as the Latin one
  EXPECTED: FAIL_IDENTITY_MIMICRY
  RESULT: FAIL
MUTATION_06:
  CLAIM: swapping u→ս in an identifier is harmless
