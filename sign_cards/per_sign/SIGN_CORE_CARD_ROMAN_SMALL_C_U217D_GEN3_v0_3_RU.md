...
DOCUMENT_ID: SIGN_CORE_CARD_ROMAN_SMALL_C_U217D_GEN3_v0_3_RU
...
TRANSLATION_NOTE: Русский оригинал (авторитетный источник). Английский документ SIGN_CORE_CARD_ROMAN_SMALL_C_U217D_GEN3_v0_3_EN — зеркало. Кодовые точки, имена полей и формулы идентичны. Гомоглиф: базовый закон — ВЫГЛЯДИТ_ОДИНАКОВО ≠ ЯВЛЯЕТСЯ_ТЕМ_ЖЕ. ЧЕРНОВИК для нашей работы (Vakhter); конвейер — отдельный проект.
...
2. META
============================================================
CARD_UID: SIGN_CORE_CARD_ROMAN_SMALL_C_U217D_GEN3_v0_3_RU
CODEPOINT: U+217D
VISIBLE_FORM: ⅽ
...
DISPLAY_NAME: римская цифра «ⅽ» малая сто (гомоглиф латинской строчной c)
...
SIGN_CATEGORY:
  - римская цифро-буквоформа «малая римская сто» (означает 100 как самостоятельную цифру; символ блока Number Forms)
  - гомоглиф латинской строчной «c» (U+0063)
  - потенциальный носитель гомоглифного спуфинга при вставке ВНУТРЬ латинского слова

WHAT_THIS_SIGN_IS_NOT:
  1. NOT_LATIN_C — римская ⅽ (U+217D) НЕ есть латинская c (U+0063); другая кодовая точка И это цифра (значение 100), а не буква
...
NOTE: римская «ⅽ» малая сто — стабильная цифро-форма без культурной прецессии функций. «Гомоглиф» — не эпоха, а свойство визуального совпадения с латинской c, сосуществующее с легитимной цифровой функцией формы. Опасность контекстна (вставка внутрь латинского слова), а не эпохальна — поэтому SEMANTIC_EPOCH_TRACKER не применяется.
LAYER_ANOMALY:
...
SAFE_CASES:
  ...
  SAFE_CASE_005:
    ...
    GUARD: ROMAN_SMALL_C_FORM ≠ AUTOMATICALLY_SPOOF
  SAFE_CASE_006:
    INPUT: "ⅽⅽ pages" (200 страниц, римская цифра)
    CONTEXT: цельный токен из римских цифр
    EXPECTED: INFO
    RISK: NONE
    GUARD: ROMAN_SMALL_C_FORM ≠ AUTOMATICALLY_SPOOF
...
RISK_CASES:
  ...
  RISK_CASE_003:
    ...
    RISK: HIGH
    ATTACK: похожий хэндл выглядит как costcoHelp, но это другой аккаунт
  ...
  RISK_CASE_005:
    ...
    INPUT: "no-reply@ⅽhase-secure.example"
  ...
  RISK_CASE_006:
    ...
    INPUT: "ⅽostⅽo"
    CONTEXT: две римские ⅽ среди латинских букв, имитирующие costco (ⅽ римская, ost/o латиница — цифра-в-слове)
    RISK: HIGH
...
CONFUSABLES:
  CONFUSABLE_001:
    ...
    RISK: CRITICAL
    RULE: LATIN_C ≠ ROMAN_SMALL_C (главная цель имперсонации; визуально идентична во многих шрифтах)
  CONFUSABLE_002:
    VISIBLE_FORM: с
    CODEPOINT: U+0441
    NAME: CYRILLIC SMALL LETTER ES
    RISK: HIGH
    RULE: CYRILLIC_ES ≠ ROMAN_SMALL_C (другой скрипт с той же формой c; усложняет детекцию)
  CONFUSABLE_003:
    VISIBLE_FORM: ｃ
    CODEPOINT: U+FF43
    NAME: FULLWIDTH LATIN SMALL LETTER C
    RISK: MEDIUM
    RULE: FULLWIDTH_SMALL_C ≠ ROMAN_SMALL_C (полноширинная латинская c; другая форма совместимости)
  CONFUSABLE_004:
    VISIBLE_FORM: 𝖼
    CODEPOINT: U+1D5BC
    NAME: MATHEMATICAL SANS-SERIF SMALL C
    RISK: MEDIUM
    RULE: MATH_SANS_SMALL_C ≠ ROMAN_SMALL_C (математически-стилизованная латинская c для обхода простых фильтров)
  CONFUSABLE_005:
    VISIBLE_FORM: 𝐜
    CODEPOINT: U+1D41C
    NAME: MATHEMATICAL BOLD SMALL C
    RISK: LOW
    RULE: MATH_BOLD_SMALL_C ≠ ROMAN_SMALL_C (жирная латинская c)
  CONFUSABLE_006:
    VISIBLE_FORM: 𝑐
    CODEPOINT: U+1D450
    NAME: MATHEMATICAL ITALIC SMALL C
    RISK: LOW
    RULE: MATH_ITALIC_SMALL_C ≠ ROMAN_SMALL_C (курсивная латинская c)
...
CONTRADICTION_GUARDS:
  ...
  CG5:
    TRIGGER: «хэндл `@ⅽostcoHelp` — тот же аккаунт, что и @costcoHelp»
...
SEQUENCE_LAYER_BOUNDARY:
  ...
  SEQUENCE_CANDIDATES:
    ...
    SC2:
      SEQUENCE: "ⅽostⅽo" (две римские ⅽ среди латинских букв)
...
CATEGORY_A: FORM_MANIPULATION (2)
  A1: замена латинской c (U+0063) на римскую ⅽ (U+217D) в домене бренда
  A2: смешение римской ⅽ с кириллической es с / математической sans-serif c для усложнения детекции
CATEGORY_B: CONTEXT_INJECTION (2)
  B1: римская ⅽ обходит латинский блок-лист ключевых слов (ⅽlick)
  B2: римская ⅽ в почтовом домене (no-reply@ⅽhase-secure.example) для фишинга
...
CATEGORY_D: SEMANTIC_MIMICRY (2)
  D1: хэндл `@ⅽostcoHelp` имитирует бренд-аккаунт
  D2: "ⅽhase-secure" — двойник в псевдо-официальном имени
CATEGORY_E: PHAGO_ENTITY_MIMICRY (2)
  E1: `ⅽisco-support.com` — имитация имени бренда (PE_001)
  E2: `@ⅽostcoHelp` — имитация официального аккаунта (PE_002)
...
MUTATION_03:
  ...
  RESULT: FAIL
MUTATION_04:
  CLAIM: ASCII-фильтр на "click" поймает "ⅽlick"
  EXPECTED: FAIL_ASCII_COVERAGE_MIMICRY
  RESULT: FAIL
MUTATION_05:
  CLAIM: хэндл `@ⅽostcoHelp` — тот же аккаунт, что и `@costcoHelp`
