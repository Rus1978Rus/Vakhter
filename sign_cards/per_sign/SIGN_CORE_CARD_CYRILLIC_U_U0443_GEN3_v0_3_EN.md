...
DOCUMENT_ID: SIGN_CORE_CARD_CYRILLIC_U_U0443_GEN3_v0_3_EN
...
TRANSLATION_NOTE: English mirror of SIGN_CORE_CARD_CYRILLIC_U_U0443_GEN3_v0_3_RU (authoritative). Codepoints, field names and formulas identical. Homoglyph sign: core law is LOOKS_SAME ≠ IS_SAME. DRAFT for our work (Vakhter); conveyor is a separate project.
...
2. META
============================================================
CARD_UID: SIGN_CORE_CARD_CYRILLIC_U_U0443_GEN3_v0_3_EN
CODEPOINT: U+0443
VISIBLE_FORM: у
...
DISPLAY_NAME: Cyrillic "у" (homoglyph of Latin y)
...
SIGN_CATEGORY:
  ...
  - homoglyph of Latin small "y" (U+0079)
  - potential carrier of homoglyph / IDN spoofing when scripts are mixed

WHAT_THIS_SIGN_IS_NOT:
  1. NOT_LATIN_Y — Cyrillic у (U+0443) is NOT Latin y (U+0079); a different codepoint
  2. NOT_SAME_STRING_AS_LATIN — a string with Cyrillic у is not machine-equal to its Latin twin
  3. NOT_BRAND_NAME_PROOF — "уoutube.com" does not prove a link to the YouTube brand
...
NOTE: Cyrillic "у" is a stable letter with no cultural precession of functions. "Homoglyph" is not an epoch but a property of visual coincidence with Latin y, co-existing with the letter's legit function. The danger is contextual (mixed scripts), not epochal — so SEMANTIC_EPOCH_TRACKER does not apply.
LAYER_ANOMALY:
...
SAFE_CASES:
  ...
  SAFE_CASE_003:
    ...
    CONTEXT: Russian words where "у" is an ordinary letter
    EXPECTED: INFO
    RISK: NONE
    GUARD: CYRILLIC_U_FORM ≠ AUTOMATICALLY_SPOOF
  SAFE_CASE_004:
  ...
  SAFE_CASE_006:
    ...
    CONTEXT: a phrase rich in "у", yet all Cyrillic
    EXPECTED: INFO
    RISK: NONE
    GUARD: CYRILLIC_U_FORM ≠ AUTOMATICALLY_SPOOF
...
RISK_CASES:
  RISK_CASE_001:
    ...
    CONTEXT: IDN/brand spoof — the token looks like youtube.com but the first char is Cyrillic
    RISK: CRITICAL
    ATTACK: replacing Latin y with Cyrillic у yields a visually identical domain the attacker registers
  ...
  RISK_CASE_003:
    ...
    CONTEXT: impersonating a security account in a chat/social platform
    RISK: HIGH
    ATTACK: the look-alike handle appears as security but is a different account
  ...
  RISK_CASE_005:
    ...
    CONTEXT: a phishing email from the "same" payment service
    RISK: HIGH
...
CONFUSABLES:
  CONFUSABLE_001:
    ...
    RISK: CRITICAL
    RULE: LATIN_Y ≠ CYRILLIC_U (the primary impersonation target; visually identical in many fonts)
  CONFUSABLE_002:
    VISIBLE_FORM: ү
    CODEPOINT: U+04AF
    NAME: CYRILLIC SMALL LETTER STRAIGHT U
    RISK: MEDIUM
    RULE: CYRILLIC_STRAIGHT_U ≠ CYRILLIC_U (same script, a related letter; distinct codepoint)
  CONFUSABLE_003:
    VISIBLE_FORM: 𝗒
    CODEPOINT: U+1D5D2
    NAME: MATHEMATICAL SANS-SERIF SMALL Y
    RISK: MEDIUM
    RULE: MATH_SANS_Y ≠ CYRILLIC_U (a math-alphanumeric styled Latin y used to dodge simple filters)
  CONFUSABLE_004:
    VISIBLE_FORM: ý
    CODEPOINT: U+00FD
    NAME: LATIN SMALL LETTER Y WITH ACUTE
    RISK: LOW
    RULE: LATIN_Y_ACUTE ≠ CYRILLIC_U (an accented Latin twin)
  CONFUSABLE_005:
    VISIBLE_FORM: ｙ
    CODEPOINT: U+FF59
    NAME: FULLWIDTH LATIN SMALL LETTER Y
    RISK: LOW
    RULE: FULLWIDTH_Y ≠ CYRILLIC_U (a full-width Latin y; a different compatibility form)
  CONFUSABLE_006:
    VISIBLE_FORM: ÿ
    CODEPOINT: U+00FF
    NAME: LATIN SMALL LETTER Y WITH DIAERESIS
    RISK: LOW
    RULE: LATIN_Y_DIAERESIS ≠ CYRILLIC_U (a decorated Latin twin)
...
CATEGORY_A: FORM_MANIPULATION (2)
  A1: substitution of Latin y (U+0079) with Cyrillic у (U+0443) in a brand domain
  A2: mixing Cyrillic у with math sans-serif y / accented Latin ý to complicate detection
CATEGORY_B: CONTEXT_INJECTION (2)
  B1: Cyrillic у bypasses a Latin-keyword blocklist (buу-now)
  B2: Cyrillic у in a mail domain (alerts@paу-verify.example) for phishing
CATEGORY_C: SEQUENCE_MANIPULATION (2)
  C1: the mixed token `уoutube` (SC1) — scripts within one word
  C2: multiple substitution `moneу-paу` (SC2) onto a target brand
CATEGORY_D: SEMANTIC_MIMICRY (2)
  D1: the handle `@securitу` imitates a service account
  D2: "paу-official" — a look-alike in a pseudo-official name
CATEGORY_E: PHAGO_ENTITY_MIMICRY (2)
  E1: `уoutube.com` — mimicry of a brand name (PE_001)
  E2: `@paуpal_support` — mimicry of an official account (PE_002)
