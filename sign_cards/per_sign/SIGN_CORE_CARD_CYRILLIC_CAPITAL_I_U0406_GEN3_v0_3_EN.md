...
DOCUMENT_ID: SIGN_CORE_CARD_CYRILLIC_CAPITAL_I_U0406_GEN3_v0_3_EN
...
TRANSLATION_NOTE: English mirror of SIGN_CORE_CARD_CYRILLIC_CAPITAL_I_U0406_GEN3_v0_3_RU (authoritative). Codepoints, field names and formulas identical. Homoglyph sign: core law is LOOKS_SAME ≠ IS_SAME. DRAFT for our work (Vakhter); conveyor is a separate project.
...
2. META
============================================================
CARD_UID: SIGN_CORE_CARD_CYRILLIC_CAPITAL_I_U0406_GEN3_v0_3_EN
CODEPOINT: U+0406
VISIBLE_FORM: І
...
DISPLAY_NAME: Cyrillic "І" (homoglyph of Latin capital I; reads /i/, Ukrainian/Belarusian)
...
SIGN_CATEGORY:
  - Cyrillic capital letter Byelorussian-Ukrainian I (sounds /i/; legitimate in Ukrainian / Belarusian scripts)
  - homoglyph of Latin capital "I" (U+0049)
  - potential carrier of homoglyph / IDN spoofing when scripts are mixed

WHAT_THIS_SIGN_IS_NOT:
  1. NOT_LATIN_CAP_I — Cyrillic І (U+0406) is NOT Latin I (U+0049); a different codepoint in a different script
  2. NOT_SAME_STRING_AS_LATIN — a string with Cyrillic І is not machine-equal to its Latin twin
  3. NOT_BRAND_NAME_PROOF — "ІNTEL" does not prove a link to the Intel brand
  ...
  8. NOT_AUTOMATICALLY_SPOOF — in single-script Ukrainian text it is normal, not an attack
...
SAFE_CASES:
  SAFE_CASE_001:
    INPUT: "Ігор і Іван" (Ihor and Ivan, Ukrainian names)
    CONTEXT: ordinary Ukrainian text (single script)
    EXPECTED: INFO
    RISK: NONE
    GUARD: CYRILLIC_CAP_I_FORM ≠ AUTOMATICALLY_SPOOF
  SAFE_CASE_002:
    INPUT: "Історія України" (the history of Ukraine)
    CONTEXT: a Ukrainian phrase where "І" is an ordinary capital
    EXPECTED: INFO
    RISK: NONE
    GUARD: CYRILLIC_CAP_I_FORM ≠ EFFECT
  SAFE_CASE_003:
    INPUT: "Індія і Іспанія" (India and Spain, Ukrainian)
    CONTEXT: single-script Cyrillic phrase
    EXPECTED: INFO
    RISK: NONE
    GUARD: CYRILLIC_CAP_I_FORM ≠ AUTOMATICALLY_SPOOF
  SAFE_CASE_004:
    INPUT: "Іній на вікні" (frost on the window, Ukrainian)
    CONTEXT: single-script Cyrillic phrase
    EXPECTED: INFO
    RISK: NONE
    GUARD: CYRILLIC_CAP_I_FORM ≠ EFFECT
  SAFE_CASE_005:
    INPUT: "Ідея і думка" (idea and thought, Ukrainian)
    CONTEXT: Ukrainian words beginning with "І"
    EXPECTED: INFO
    RISK: NONE
    GUARD: CYRILLIC_CAP_I_FORM ≠ AUTOMATICALLY_SPOOF
  SAFE_CASE_006:
    INPUT: "Івано-Франківськ" (Ivano-Frankivsk, a Ukrainian city)
    CONTEXT: a single-script Cyrillic place name
    EXPECTED: INFO
    RISK: NONE
    GUARD: CYRILLIC_CAP_I_FORM ≠ AUTOMATICALLY_SPOOF
...
RISK_CASES:
  RISK_CASE_001:
    ...
    INPUT: "ІNTEL-drivers.com" (Cyrillic І at the start of a brand)
    CONTEXT: IDN/brand spoof — the token looks like INTEL but the first char is Cyrillic
    RISK: CRITICAL
    ATTACK: replacing Latin I with Cyrillic І yields a visually identical brand the attacker registers
    GUARD: CYRILLIC_CAP_I_FORM ≠ LATIN_CAP_I
  RISK_CASE_002:
    NAME: ALLCAPS_BRAND_SUBSTITUTION
    INPUT: "ІRS-refund-alert.com" (Cyrillic І in an all-caps brand)
    CONTEXT: leading substitution in an all-caps brand phrase
    RISK: CRITICAL
    ATTACK: the string is machine-≠ IRS, but a human sees no difference
    GUARD: CYRILLIC_CAP_I_FORM ≠ DOMAIN_VALIDITY_PROOF
  RISK_CASE_003:
    NAME: HANDLE_IMPERSONATION
    INPUT: "@ІnstacartHelp" (Cyrillic І in a handle)
    CONTEXT: impersonating a brand account in a chat/social platform
    RISK: HIGH
    ATTACK: the look-alike handle appears as InstacartHelp but is a different account
    GUARD: CYRILLIC_CAP_I_FORM ≠ VERIFICATION
  RISK_CASE_004:
    NAME: FILTER_BYPASS_KEYWORD
    INPUT: "ІNFO" (Cyrillic І bypasses an "INFO" blocklist)
  ...
  RISK_CASE_005:
    ...
    INPUT: "support@Іnstagram-secure.example" (Cyrillic І in the mail domain)
  ...
  RISK_CASE_006:
    ...
    INPUT: "ІNFІNITI" (two Cyrillic І among Latin letters, imitating INFINITI)
    CONTEXT: multiple substitutions in one all-caps token (І Cyrillic, NF/NITI Latin — a script mix)
    RISK: HIGH
...
CONFUSABLES:
  CONFUSABLE_001:
    ...
    RISK: CRITICAL
    RULE: LATIN_CAP_I ≠ CYRILLIC_CAP_I (the primary impersonation target; visually identical in many fonts)
  CONFUSABLE_002:
    VISIBLE_FORM: Ι
    CODEPOINT: U+0399
    NAME: GREEK CAPITAL LETTER IOTA
    RISK: HIGH
    RULE: GREEK_CAP_IOTA ≠ CYRILLIC_CAP_I (a third script with the same I-shape; complicates detection)
  CONFUSABLE_003:
    VISIBLE_FORM: Ⅰ
    CODEPOINT: U+2160
    NAME: ROMAN NUMERAL ONE
    RISK: MEDIUM
    RULE: ROMAN_NUMERAL_ONE ≠ CYRILLIC_CAP_I (a numeral form with the same I-shape used to dodge simple filters)
  CONFUSABLE_004:
    VISIBLE_FORM: Ｉ
    CODEPOINT: U+FF29
    NAME: FULLWIDTH LATIN CAPITAL LETTER I
    RISK: MEDIUM
    RULE: FULLWIDTH_CAP_I ≠ CYRILLIC_CAP_I (a full-width Latin I; a different compatibility form)
  CONFUSABLE_005:
    VISIBLE_FORM: 𝗜
    CODEPOINT: U+1D5DC
    NAME: MATHEMATICAL SANS-SERIF BOLD CAPITAL I
    RISK: LOW
    RULE: MATH_SANS_CAP_I ≠ CYRILLIC_CAP_I (a math-alphanumeric styled Latin I)
  CONFUSABLE_006:
    VISIBLE_FORM: 𝐈
    CODEPOINT: U+1D408
    NAME: MATHEMATICAL BOLD CAPITAL I
    RISK: LOW
    RULE: MATH_BOLD_CAP_I ≠ CYRILLIC_CAP_I (a bold-styled Latin I)

CONTRADICTION_GUARDS:
  CG1:
    TRIGGER: "the brand `ІNTEL` is Intel"
    RESPONSE: CYRILLIC_CAP_I_FORM ≠ LATIN_CAP_I
    RULE: the first char is Cyrillic І; the registrable domain/name is different — DNS decides, not the eye
  CG2:
  ...
  CG3:
    ...
    RULE: in single-script Ukrainian text the letter is legit; the spoof is a MIX of scripts in one token
  CG4:
  ...
  CG5:
    TRIGGER: "the handle `@ІnstacartHelp` is the same account as @InstacartHelp"
...
CATEGORY_A: FORM_MANIPULATION (2)
  A1: substitution of Latin I (U+0049) with Cyrillic І (U+0406) in a brand domain
  A2: mixing Cyrillic І with Greek Iota Ι / Roman numeral one to complicate detection
CATEGORY_B: CONTEXT_INJECTION (2)
  B1: Cyrillic І bypasses a Latin-keyword blocklist (ІNFO)
  B2: Cyrillic І in a mail domain (support@Іnstagram-secure.example) for phishing
CATEGORY_C: SEQUENCE_MANIPULATION (2)
  C1: the mixed token `ІNTEL` (SC1) — scripts within one word
  C2: multiple substitution `ІNFІNITI` (SC2) onto a target brand
CATEGORY_D: SEMANTIC_MIMICRY (2)
  D1: the handle `@ІnstacartHelp` imitates a brand account
  D2: "Іnstagram-secure" — a look-alike in a pseudo-official name
CATEGORY_E: PHAGO_ENTITY_MIMICRY (2)
  E1: `ІNTEL-drivers.com` — mimicry of a brand name (PE_001)
  E2: `@ІnstacartHelp` — mimicry of an official account (PE_002)
...
============================================================
MUTATION_01:
  CLAIM: `ІNTEL-drivers.com` with Cyrillic І is Intel's domain
...
MUTATION_03:
  ...
  RESULT: FAIL
MUTATION_04:
  CLAIM: an ASCII filter on "INFO" will catch "ІNFO"
  EXPECTED: FAIL_ASCII_COVERAGE_MIMICRY
  RESULT: FAIL
MUTATION_05:
  CLAIM: the handle `@ІnstacartHelp` is the same account as `@InstacartHelp`
...
============================================================
OQ1:
  QUESTION: how to distinguish legit single-script Ukrainian text from a spoof without false positives?
