...
DOCUMENT_ID: SIGN_CORE_CARD_ARMENIAN_VO_U0578_GEN3_v0_3_EN
...
TRANSLATION_NOTE: English mirror of SIGN_CORE_CARD_ARMENIAN_VO_U0578_GEN3_v0_3_RU (authoritative). Codepoints, field names and formulas identical. Homoglyph sign: core law is LOOKS_SAME ≠ IS_SAME. DRAFT for our work (Vakhter); conveyor is a separate project.
...
2. META
============================================================
CARD_UID: SIGN_CORE_CARD_ARMENIAN_VO_U0578_GEN3_v0_3_EN
CODEPOINT: U+0578
VISIBLE_FORM: ո
...
DISPLAY_NAME: Armenian "ո" vo (homoglyph of Latin small n)
...
SIGN_CATEGORY:
  - Armenian vo — a vowel letter of the Armenian alphabet
  - homoglyph of Latin small "n" (U+006E)
  - potential carrier of homoglyph / IDN spoofing when scripts are mixed

WHAT_THIS_SIGN_IS_NOT:
  1. NOT_LATIN_SMALL_N — Armenian ո (U+0578) is NOT Latin n (U+006E); a different codepoint and the vowel /o/ (word-initially /vo/)
  ...
  12. NOT_INTERCHANGEABLE_IN_IDENTIFIERS — in an identifier/domain, n→ո changes the entity
...
NOTE: Armenian the letter vo is a stable letter with no cultural precession of functions. "Homoglyph" is not an epoch but a property of visual coincidence with Latin n, co-existing with the letter's legit function. The danger is contextual (mixed scripts), not epochal — so SEMANTIC_EPOCH_TRACKER does not apply.
LAYER_ANOMALY:
  ABSENT_PHYSICAL_LAYER: YES, level L1 (physical gesture)
  NOTE: a letter is a written sign with no gestural predecessor; the letter vo is a written glyph of the Armenian script.
...
SAFE_CASES:
  SAFE_CASE_001:
    INPUT: "որդի" (Armenian (a son))
    CONTEXT: single-script native text
    EXPECTED: INFO
    RISK: NONE
    GUARD: ARMENIAN_VO_FORM ≠ AUTOMATICALLY_SPOOF
  SAFE_CASE_002:
    INPUT: "ոսկի" (Armenian (gold))
    CONTEXT: single-script native text
    EXPECTED: INFO
    RISK: NONE
    GUARD: ARMENIAN_VO_FORM ≠ EFFECT
  SAFE_CASE_003:
    INPUT: "ոտք" (Armenian (a foot))
    CONTEXT: single-script native text
    EXPECTED: INFO
    RISK: NONE
    GUARD: ARMENIAN_VO_FORM ≠ AUTOMATICALLY_SPOOF
  SAFE_CASE_004:
    INPUT: "ով" (Armenian (who))
    CONTEXT: single-script native text
    EXPECTED: INFO
    RISK: NONE
    GUARD: ARMENIAN_VO_FORM ≠ EFFECT
  SAFE_CASE_005:
    INPUT: "որս" (Armenian (a hunt))
    CONTEXT: single-script native text
    EXPECTED: INFO
    RISK: NONE
    GUARD: ARMENIAN_VO_FORM ≠ AUTOMATICALLY_SPOOF
  SAFE_CASE_006:
    INPUT: "ողջ" (Armenian (whole/alive))
    CONTEXT: single-script native text
    EXPECTED: INFO
    RISK: NONE
    GUARD: ARMENIAN_VO_FORM ≠ AUTOMATICALLY_SPOOF
...
RISK_CASES:
  RISK_CASE_001:
    ...
    INPUT: "ոetflix-login.com" (IDN/brand spoof — the token looks like netflix but a letter is Armenian)
    CONTEXT: IDN/brand spoof — the token looks like netflix but a letter is Armenian
    RISK: CRITICAL
    ATTACK: replacing Latin n with Armenian ո yields a visually identical brand the attacker registers
    GUARD: ARMENIAN_VO_FORM ≠ LATIN_SMALL_N
  RISK_CASE_002:
    NAME: BRAND_DOMAIN_SUBSTITUTION
    INPUT: "liոkedin" (a look-alike inside the token `linkedin`)
    CONTEXT: a look-alike inside the token `linkedin`
    RISK: CRITICAL
    ATTACK: the string is machine-≠ linkedin, but a human sees no difference
    GUARD: ARMENIAN_VO_FORM ≠ DOMAIN_VALIDITY_PROOF
  RISK_CASE_003:
    NAME: HANDLE_IMPERSONATION
    INPUT: "@ոortonSupport" (impersonating a brand account in a chat/social platform)
    CONTEXT: impersonating a brand account in a chat/social platform
    RISK: HIGH
    ATTACK: the look-alike handle appears as nortonSupport but is a different account
    GUARD: ARMENIAN_VO_FORM ≠ VERIFICATION
  RISK_CASE_004:
    NAME: FILTER_BYPASS_KEYWORD
    INPUT: "logiո" (bypassing a text filter looking for the Latin word `login`)
  ...
  RISK_CASE_005:
    ...
    INPUT: "billing@ոorton-secure.example" (a phishing email from the "same" company)
  ...
  RISK_CASE_006:
    ...
    INPUT: "biոance-billing.com" (another target brand (binance) opened with the look-alike)
    CONTEXT: another target brand (binance) opened with the look-alike
    RISK: HIGH
...
CONFUSABLES:
  CONFUSABLE_001:
    ...
    RISK: CRITICAL
    RULE: LATIN_SMALL_LETTER_N ≠ ARMENIAN_VO_FORM (the primary impersonation target; visually identical in many fonts)
  CONFUSABLE_002:
    VISIBLE_FORM: ｎ
    CODEPOINT: U+FF4E
    NAME: FULLWIDTH LATIN SMALL LETTER N
    RISK: MEDIUM
    RULE: FULLWIDTH_LATIN_SMALL_LETTER_N ≠ ARMENIAN_VO_FORM (a full-width Latin form; a different compatibility form)
  CONFUSABLE_003:
    VISIBLE_FORM: 𝗇
    CODEPOINT: U+1D5C7
    NAME: MATHEMATICAL SANS-SERIF SMALL N
    RISK: MEDIUM
    RULE: MATHEMATICAL_SANS_SERIF_SMALL_N ≠ ARMENIAN_VO_FORM (a math-alphanumeric styled Latin letter used to dodge simple filters)
  CONFUSABLE_004:
    VISIBLE_FORM: 𝐧
    CODEPOINT: U+1D427
    NAME: MATHEMATICAL BOLD SMALL N
    RISK: LOW
    RULE: MATHEMATICAL_BOLD_SMALL_N ≠ ARMENIAN_VO_FORM (a bold-styled Latin letter)
  CONFUSABLE_005:
    VISIBLE_FORM: 𝑛
    CODEPOINT: U+1D45B
    NAME: MATHEMATICAL ITALIC SMALL N
    RISK: LOW
    RULE: MATHEMATICAL_ITALIC_SMALL_N ≠ ARMENIAN_VO_FORM (an italic-styled Latin letter)
  CONFUSABLE_006:
    VISIBLE_FORM: 𝚗
    CODEPOINT: U+1D697
    NAME: MATHEMATICAL MONOSPACE SMALL N
    RISK: LOW
    RULE: MATHEMATICAL_MONOSPACE_SMALL_N ≠ ARMENIAN_VO_FORM (a monospace-styled Latin letter)

CONTRADICTION_GUARDS:
  CG1:
    TRIGGER: "the brand `ոetflix` is Netflix"
    RESPONSE: ARMENIAN_VO_FORM ≠ LATIN_SMALL_N
    RULE: the letter is Armenian ո (and the vowel /o/ (word-initially /vo/)); the registrable domain/name is different — DNS decides, not the eye
  CG2:
  ...
  CG5:
    TRIGGER: "the handle `@ոortonSupport` is the same account as the Latin one"
    RESPONSE: ARMENIAN_VO_FORM ≠ VERIFICATION
    RULE: visual similarity does not identify an account
  CG6:
    TRIGGER: "swapping n→ո in an identifier is harmless"
...
SEQUENCE_LAYER_BOUNDARY:
  ...
  SEQUENCE_CANDIDATES:
    SC1:
      SEQUENCE: "ոetflix-login.com" (Armenian ո + Latin in one token)
      NAME: MIXED_SCRIPT_TOKEN
      RISK_LEVEL: CRITICAL
      POSSIBLE_CONTEXTS: IDN/brand spoof; the key signal is a SCRIPT MIX within one token
      REQUIRES_SEQUENCE_INTEGRATOR: YES
    SC2:
      SEQUENCE: "biոance-billing.com" (capital-initial look-alike on a target brand)
...
CATEGORY_A: FORM_MANIPULATION (2)
  A1: substitution of Latin n (U+006E) with Armenian ո (U+0578) in a brand domain
  A2: mixing ո with math/full-width Latin n to complicate detection
CATEGORY_B: CONTEXT_INJECTION (2)
  B1: ո bypasses a Latin-keyword blocklist (logiո)
  B2: ո in a mail domain (billing@ոorton-secure.example) for phishing
CATEGORY_C: SEQUENCE_MANIPULATION (2)
  C1: the mixed token `ոetflix-login.com` (SC1) — scripts within one word
  C2: opening/multiple substitution `biոance-billing.com` (SC2) onto a target brand
CATEGORY_D: SEMANTIC_MIMICRY (2)
  D1: the handle `@ոortonSupport` imitates a brand account
  D2: `billing@ոorton-secure.example` — a look-alike in a pseudo-official name
CATEGORY_E: PHAGO_ENTITY_MIMICRY (2)
  E1: `ոetflix-login.com` — mimicry of a brand name (PE_001)
  E2: `@ոortonSupport` — mimicry of an official account (PE_002)
...
============================================================
MUTATION_01:
  CLAIM: `ոetflix-login.com` with ո is the real brand's domain
...
MUTATION_03:
  ...
  RESULT: FAIL
MUTATION_04:
  CLAIM: an ASCII filter on the Latin word will catch `logiո`
  EXPECTED: FAIL_ASCII_COVERAGE_MIMICRY
  RESULT: FAIL
MUTATION_05:
  CLAIM: the handle `@ոortonSupport` is the same account as the Latin one
  EXPECTED: FAIL_IDENTITY_MIMICRY
  RESULT: FAIL
MUTATION_06:
  CLAIM: swapping n→ո in an identifier is harmless
