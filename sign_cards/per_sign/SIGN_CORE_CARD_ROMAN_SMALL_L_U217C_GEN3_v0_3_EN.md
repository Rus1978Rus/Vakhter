...
DOCUMENT_ID: SIGN_CORE_CARD_ROMAN_SMALL_L_U217C_GEN3_v0_3_EN
...
TRANSLATION_NOTE: English mirror of SIGN_CORE_CARD_ROMAN_SMALL_L_U217C_GEN3_v0_3_RU (authoritative). Codepoints, field names and formulas identical. Homoglyph sign: core law is LOOKS_SAME ≠ IS_SAME. DRAFT for our work (Vakhter); conveyor is a separate project.
...
2. META
============================================================
CARD_UID: SIGN_CORE_CARD_ROMAN_SMALL_L_U217C_GEN3_v0_3_EN
CODEPOINT: U+217C
VISIBLE_FORM: ⅼ
...
DISPLAY_NAME: Roman numeral "ⅼ" small fifty (homoglyph of Latin small l)
...
SIGN_CATEGORY:
  - Roman-numeral letter-form "small roman numeral fifty" (means 50 as a standalone numeral; a Number Forms character)
  - homoglyph of Latin small "l" (U+006C)
  - potential carrier of homoglyph spoofing when mixed INTO a Latin word

WHAT_THIS_SIGN_IS_NOT:
  1. NOT_LATIN_L — Roman ⅼ (U+217C) is NOT Latin l (U+006C); a different codepoint AND a numeral (value 50), not a letter
...
SAFE_CASES:
  ...
  SAFE_CASE_002:
    INPUT: "ⅼ candles" (50 candles)
    CONTEXT: a Roman numeral as a whole token, not inside a word
    EXPECTED: INFO
    RISK: NONE
    GUARD: ROMAN_SMALL_L_FORM ≠ EFFECT
  SAFE_CASE_003:
    INPUT: "ⅽⅼ years" (150 years, Roman numeral)
    CONTEXT: an all-Roman-numeral token
    EXPECTED: INFO
    RISK: NONE
    GUARD: ROMAN_SMALL_L_FORM ≠ AUTOMATICALLY_SPOOF
  SAFE_CASE_004:
    INPUT: "chapter ⅹⅼ" (chapter 40, Roman numeral)
    CONTEXT: an all-Roman-numeral token beside a Latin word
    EXPECTED: INFO
    RISK: NONE
    GUARD: ROMAN_SMALL_L_FORM ≠ EFFECT
  SAFE_CASE_005:
    INPUT: "ⅼⅹ pages" (60 pages, Roman numeral)
    CONTEXT: an all-Roman-numeral token
    EXPECTED: INFO
    RISK: NONE
    GUARD: ROMAN_SMALL_L_FORM ≠ AUTOMATICALLY_SPOOF
  SAFE_CASE_006:
    INPUT: "verse ⅼ" (verse 50, Roman numeral)
...
RISK_CASES:
  RISK_CASE_001:
    ...
    INPUT: "paypaⅼ.com" (Roman ⅼ inside a brand word)
    CONTEXT: IDN/brand spoof — the token looks like paypal but the last letter is a Roman numeral
    RISK: CRITICAL
  ...
  RISK_CASE_002:
    ...
    INPUT: "ⅼinkedin-login.com" (Roman ⅼ at the start of a brand)
  ...
  RISK_CASE_003:
    ...
    INPUT: "@ⅼyftSupport" (Roman ⅼ in a handle)
    CONTEXT: impersonating a brand account in a chat/social platform
    RISK: HIGH
    ATTACK: the look-alike handle appears as lyftSupport but is a different account
  ...
  RISK_CASE_006:
    ...
    INPUT: "ⅼuⅼu" (two Roman ⅼ around Latin letters, imitating lulu)
    CONTEXT: multiple substitutions in one token (ⅼ Roman, u Latin — a numeral-in-word mix)
    RISK: HIGH
...
CONFUSABLES:
  CONFUSABLE_001:
    ...
    RISK: CRITICAL
    RULE: LATIN_L ≠ ROMAN_SMALL_L (the primary impersonation target; visually identical in many fonts)
  CONFUSABLE_002:
    VISIBLE_FORM: ℓ
    CODEPOINT: U+2113
    NAME: SCRIPT SMALL L
    RISK: MEDIUM
    RULE: SCRIPT_SMALL_L ≠ ROMAN_SMALL_L (a letter-like script l used to dodge simple filters)
  CONFUSABLE_003:
    VISIBLE_FORM: ｌ
    CODEPOINT: U+FF4C
    NAME: FULLWIDTH LATIN SMALL LETTER L
    RISK: MEDIUM
    RULE: FULLWIDTH_SMALL_L ≠ ROMAN_SMALL_L (a full-width Latin l; a different compatibility form)
  CONFUSABLE_004:
    VISIBLE_FORM: 𝗅
    CODEPOINT: U+1D5C5
    NAME: MATHEMATICAL SANS-SERIF SMALL L
    RISK: MEDIUM
    RULE: MATH_SANS_SMALL_L ≠ ROMAN_SMALL_L (a math-alphanumeric styled Latin l)
  CONFUSABLE_005:
    VISIBLE_FORM: 𝐥
    CODEPOINT: U+1D425
    NAME: MATHEMATICAL BOLD SMALL L
    RISK: LOW
    RULE: MATH_BOLD_SMALL_L ≠ ROMAN_SMALL_L (a bold-styled Latin l)
  CONFUSABLE_006:
    VISIBLE_FORM: 𝑙
    CODEPOINT: U+1D459
    NAME: MATHEMATICAL ITALIC SMALL L
    RISK: LOW
    RULE: MATH_ITALIC_SMALL_L ≠ ROMAN_SMALL_L (an italic-styled Latin l)

CONTRADICTION_GUARDS:
  CG1:
    TRIGGER: "the brand `paypaⅼ` is PayPal"
    RESPONSE: ROMAN_SMALL_L_FORM ≠ LATIN_L
    RULE: the last char is a Roman numeral (50); the registrable domain/name is different — DNS decides, not the eye
  CG2:
  ...
  CG5:
    TRIGGER: "the handle `@ⅼyftSupport` is the same account as @lyftSupport"
...
SEQUENCE_LAYER_BOUNDARY:
  ...
  SEQUENCE_CANDIDATES:
    SC1:
      SEQUENCE: "paypaⅼ" (Latin word + Roman ⅼ in one token)
    ...
    SC2:
      SEQUENCE: "ⅼuⅼu" (two Roman ⅼ among Latin letters)
...
CATEGORY_A: FORM_MANIPULATION (2)
  ...
  A2: mixing Roman ⅼ with script small l ℓ / math sans-serif l to complicate detection
...
CATEGORY_C: SEQUENCE_MANIPULATION (2)
  C1: the numeral-in-word token `paypaⅼ` (SC1) — a Roman form inside one word
  C2: multiple substitution `ⅼuⅼu` (SC2) onto a target brand
CATEGORY_D: SEMANTIC_MIMICRY (2)
  D1: the handle `@ⅼyftSupport` imitates a brand account
  D2: "ⅼloyds-secure" — a look-alike in a pseudo-official name
CATEGORY_E: PHAGO_ENTITY_MIMICRY (2)
  E1: `paypaⅼ.com` — mimicry of a brand name (PE_001)
  E2: `@ⅼyftSupport` — mimicry of an official account (PE_002)
...
MUTATION_04:
  ...
  RESULT: FAIL
MUTATION_05:
  CLAIM: the handle `@ⅼyftSupport` is the same account as `@lyftSupport`
