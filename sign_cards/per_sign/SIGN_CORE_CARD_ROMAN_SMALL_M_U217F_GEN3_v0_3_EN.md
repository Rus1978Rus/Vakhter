...
DOCUMENT_ID: SIGN_CORE_CARD_ROMAN_SMALL_M_U217F_GEN3_v0_3_EN
...
TRANSLATION_NOTE: English mirror of SIGN_CORE_CARD_ROMAN_SMALL_M_U217F_GEN3_v0_3_RU (authoritative). Codepoints, field names and formulas identical. Homoglyph sign: core law is LOOKS_SAME ≠ IS_SAME. DRAFT for our work (Vakhter); conveyor is a separate project.
...
2. META
============================================================
CARD_UID: SIGN_CORE_CARD_ROMAN_SMALL_M_U217F_GEN3_v0_3_EN
CODEPOINT: U+217F
VISIBLE_FORM: ⅿ
...
DISPLAY_NAME: Roman numeral "ⅿ" small one thousand (homoglyph of Latin small m)
...
SIGN_CATEGORY:
  - Roman-numeral letter-form "small roman numeral one thousand" (means 1000 as a standalone numeral; a Number Forms character)
  - homoglyph of Latin small "m" (U+006D)
  - potential carrier of homoglyph spoofing when mixed INTO a Latin word

WHAT_THIS_SIGN_IS_NOT:
  1. NOT_LATIN_M — Roman ⅿ (U+217F) is NOT Latin m (U+006D); a different codepoint AND a numeral (value 1000), not a letter
  2. NOT_SAME_STRING_AS_LATIN — a string with Roman ⅿ is not machine-equal to its Latin twin
  3. NOT_BRAND_NAME_PROOF — "ⅿcdonalds" does not prove a link to the McDonald's brand
  ...
  12. NOT_INTERCHANGEABLE_IN_IDENTIFIERS — in an identifier/domain, m→ⅿ changes the entity
...
NOTE: Roman "ⅿ" small one thousand is a stable numeral form with no cultural precession of functions. "Homoglyph" is not an epoch but a property of visual coincidence with Latin m, co-existing with the form's legit numeral function. The danger is contextual (embedded in a Latin word), not epochal — so SEMANTIC_EPOCH_TRACKER does not apply.
LAYER_ANOMALY:
...
SAFE_CASES:
  ...
  SAFE_CASE_003:
    ...
    GUARD: ROMAN_SMALL_M_FORM ≠ AUTOMATICALLY_SPOOF
  SAFE_CASE_004:
    INPUT: "chapter ⅿ" (chapter 1000, Roman numeral)
    CONTEXT: an all-Roman-numeral token beside a Latin word
    EXPECTED: INFO
    RISK: NONE
    GUARD: ROMAN_SMALL_M_FORM ≠ EFFECT
  SAFE_CASE_005:
    INPUT: "ⅿⅽ items" (1100 items, Roman numeral)
    CONTEXT: an all-Roman-numeral token
    EXPECTED: INFO
    RISK: NONE
    GUARD: ROMAN_SMALL_M_FORM ≠ AUTOMATICALLY_SPOOF
  SAFE_CASE_006:
    INPUT: "ⅿⅿ soldiers" (2000 soldiers, Roman numeral)
    CONTEXT: an all-Roman-numeral token
    EXPECTED: INFO
    RISK: NONE
    GUARD: ROMAN_SMALL_M_FORM ≠ AUTOMATICALLY_SPOOF
...
RISK_CASES:
  RISK_CASE_001:
    ...
    INPUT: "ⅿcdonalds-promo.com" (Roman ⅿ at the start of a brand)
    CONTEXT: IDN/brand spoof — the token looks like mcdonalds but the first char is a Roman numeral
    RISK: CRITICAL
    ATTACK: replacing Latin m with Roman ⅿ yields a visually identical brand the attacker registers
    GUARD: ROMAN_SMALL_M_FORM ≠ LATIN_M
  RISK_CASE_002:
    NAME: MIDTOKEN_BRAND_SUBSTITUTION
    INPUT: "aⅿazon-account.com" (Roman ⅿ inside a brand)
    CONTEXT: mid-token substitution in a brand phrase
    RISK: CRITICAL
    ATTACK: the string is machine-≠ amazon, but a human sees no difference
    GUARD: ROMAN_SMALL_M_FORM ≠ DOMAIN_VALIDITY_PROOF
  RISK_CASE_003:
    NAME: HANDLE_IMPERSONATION
    INPUT: "@ⅿetaSupport" (Roman ⅿ in a handle)
    CONTEXT: impersonating a brand account in a chat/social platform
    RISK: HIGH
    ATTACK: the look-alike handle appears as metaSupport but is a different account
    GUARD: ROMAN_SMALL_M_FORM ≠ VERIFICATION
  RISK_CASE_004:
    NAME: FILTER_BYPASS_KEYWORD
    INPUT: "ⅿalware" (Roman ⅿ bypasses a "malware" blocklist)
  ...
  RISK_CASE_005:
    ...
    INPUT: "no-reply@ⅿicrosoft-secure.example" (Roman ⅿ in the mail domain)
  ...
  RISK_CASE_006:
    ...
    INPUT: "ⅿaxiⅿ" (two Roman ⅿ around Latin letters, imitating maxim)
    CONTEXT: multiple substitutions in one token (ⅿ Roman, axi Latin — a numeral-in-word mix)
    RISK: HIGH
...
CONFUSABLES:
  CONFUSABLE_001:
    ...
    RISK: CRITICAL
    RULE: LATIN_M ≠ ROMAN_SMALL_M (the primary impersonation target; visually identical in many fonts)
  CONFUSABLE_002:
    VISIBLE_FORM: ｍ
    CODEPOINT: U+FF4D
    NAME: FULLWIDTH LATIN SMALL LETTER M
    RISK: MEDIUM
    RULE: FULLWIDTH_SMALL_M ≠ ROMAN_SMALL_M (a full-width Latin m; a different compatibility form)
  CONFUSABLE_003:
    VISIBLE_FORM: 𝗆
    CODEPOINT: U+1D5C6
    NAME: MATHEMATICAL SANS-SERIF SMALL M
    RISK: MEDIUM
    RULE: MATH_SANS_SMALL_M ≠ ROMAN_SMALL_M (a math-alphanumeric styled Latin m used to dodge simple filters)
  CONFUSABLE_004:
    VISIBLE_FORM: 𝕞
    CODEPOINT: U+1D55E
    NAME: MATHEMATICAL DOUBLE-STRUCK SMALL M
    RISK: MEDIUM
    RULE: DOUBLE_STRUCK_M ≠ ROMAN_SMALL_M (a double-struck styled Latin m)
  CONFUSABLE_005:
    VISIBLE_FORM: 𝐦
    CODEPOINT: U+1D426
    NAME: MATHEMATICAL BOLD SMALL M
    RISK: LOW
    RULE: MATH_BOLD_SMALL_M ≠ ROMAN_SMALL_M (a bold-styled Latin m)
  CONFUSABLE_006:
    VISIBLE_FORM: 𝑚
    CODEPOINT: U+1D45A
    NAME: MATHEMATICAL ITALIC SMALL M
    RISK: LOW
    RULE: MATH_ITALIC_SMALL_M ≠ ROMAN_SMALL_M (an italic-styled Latin m)

CONTRADICTION_GUARDS:
  CG1:
    TRIGGER: "the brand `ⅿcdonalds` is McDonald's"
    RESPONSE: ROMAN_SMALL_M_FORM ≠ LATIN_M
    RULE: the first char is a Roman numeral (1000); the registrable domain/name is different — DNS decides, not the eye
  CG2:
    TRIGGER: "a string with Roman ⅿ equals its Latin spelling"
    RESPONSE: ROMAN_SMALL_M_FORM ≠ SAME_CODEPOINT_AS_LATIN
    RULE: different codepoints → machine-different strings
  CG3:
    TRIGGER: "any Roman ⅿ in text is an attack"
  ...
  CG4:
    ...
    RULE: Roman ⅿ is outside ASCII; a Latin filter will not match it
  CG5:
    TRIGGER: "the handle `@ⅿetaSupport` is the same account as @metaSupport"
    RESPONSE: ROMAN_SMALL_M_FORM ≠ VERIFICATION
    RULE: visual similarity does not identify an account
  CG6:
    TRIGGER: "swapping m→ⅿ in an identifier is harmless"
...
SEQUENCE_LAYER_BOUNDARY:
  ...
  SEQUENCE_CANDIDATES:
    SC1:
      SEQUENCE: "ⅿcdonalds" (Roman ⅿ + Latin in one token)
    ...
    SC2:
      SEQUENCE: "ⅿaxiⅿ" (two Roman ⅿ among Latin letters)
...
PHAGO_ENTITY_MIMICRY:
  PE_001:
    ...
    RISK: CRITICAL
    NOTE: Roman ⅿ mimics the NAME itself of a verified brand (not merely structure) — direct mimicry of an entity's existence. This is why the registry flags the sign PHAGO ●; commercial lookalike defenses often miss this class.
  PE_002:
    INPUT: "@ⅿetaSupport"
    TYPE: PHAGO_ENTITY_MIMICRY
    RISK: HIGH
...
CATEGORY_A: FORM_MANIPULATION (2)
  A1: substitution of Latin m (U+006D) with Roman ⅿ (U+217F) in a brand domain
  A2: mixing Roman ⅿ with double-struck 𝕞 / math sans-serif m to complicate detection
CATEGORY_B: CONTEXT_INJECTION (2)
  B1: Roman ⅿ bypasses a Latin-keyword blocklist (ⅿalware)
  B2: Roman ⅿ in a mail domain (no-reply@ⅿicrosoft-secure.example) for phishing
CATEGORY_C: SEQUENCE_MANIPULATION (2)
  C1: the numeral-in-word token `ⅿcdonalds` (SC1) — a Roman form inside one word
  C2: multiple substitution `ⅿaxiⅿ` (SC2) onto a target brand
CATEGORY_D: SEMANTIC_MIMICRY (2)
  D1: the handle `@ⅿetaSupport` imitates a brand account
  D2: "ⅿicrosoft-secure" — a look-alike in a pseudo-official name
CATEGORY_E: PHAGO_ENTITY_MIMICRY (2)
  E1: `ⅿcdonalds-promo.com` — mimicry of a brand name (PE_001)
  E2: `@ⅿetaSupport` — mimicry of an official account (PE_002)
...
============================================================
MUTATION_01:
  CLAIM: `ⅿcdonalds-promo.com` with Roman ⅿ is McDonald's domain
  EXPECTED: FAIL_BRAND_NAME_MIMICRY
  RESULT: FAIL
MUTATION_02:
  CLAIM: a string with Roman ⅿ is machine-equal to its Latin spelling
  EXPECTED: FAIL_CODEPOINT_IDENTITY_MIMICRY
  RESULT: FAIL
MUTATION_03:
  CLAIM: any Roman ⅿ in text is an attack
  EXPECTED: FAIL_OVERBLOCK_MIMICRY (the inverse error: a standalone Roman numeral is not a spoof)
  RESULT: FAIL
MUTATION_04:
  CLAIM: an ASCII filter on "malware" will catch "ⅿalware"
  EXPECTED: FAIL_ASCII_COVERAGE_MIMICRY
  RESULT: FAIL
MUTATION_05:
  CLAIM: the handle `@ⅿetaSupport` is the same account as `@metaSupport`
  EXPECTED: FAIL_IDENTITY_MIMICRY
  RESULT: FAIL
MUTATION_06:
  CLAIM: swapping m→ⅿ in an identifier is harmless
