...
DOCUMENT_ID: SIGN_CORE_CARD_CYRILLIC_ER_U0440_GEN3_v0_3_EN
...
TRANSLATION_NOTE: English mirror of SIGN_CORE_CARD_CYRILLIC_ER_U0440_GEN3_v0_3_RU (authoritative). Codepoints, field names and formulas identical. Homoglyph sign: core law is LOOKS_SAME ≠ IS_SAME. DRAFT for our work (Vakhter); conveyor is a separate project.
...
2. META
============================================================
CARD_UID: SIGN_CORE_CARD_CYRILLIC_ER_U0440_GEN3_v0_3_EN
CODEPOINT: U+0440
VISIBLE_FORM: р
...
DISPLAY_NAME: Cyrillic "р" (homoglyph of Latin p)
...
WHAT_THIS_SIGN_IS_NOT:
  1. NOT_LATIN_P — Cyrillic р (U+0440) is NOT Latin p (U+0070); a different codepoint
  2. NOT_SAME_STRING_AS_LATIN — a string with Cyrillic р is not machine-equal to its Latin twin
  3. NOT_BRAND_NAME_PROOF — "раypal" does not prove a link to the PayPal brand
...
NOTE: Cyrillic "р" is a stable letter with no cultural precession of functions. "Homoglyph" is not an epoch but a property of visual coincidence with Latin p, co-existing with the letter's legit function. The danger is contextual (mixed scripts), not epochal — so SEMANTIC_EPOCH_TRACKER does not apply.
LAYER_ANOMALY:
...
SAFE_CASES:
  ...
  SAFE_CASE_003:
    ...
    CONTEXT: Russian words where "р" is an ordinary letter
    EXPECTED: INFO
    RISK: NONE
    GUARD: CYRILLIC_ER_FORM ≠ AUTOMATICALLY_SPOOF
  SAFE_CASE_004:
    INPUT: "город Пермь" (the city of Perm)
    CONTEXT: a Russian place name
    EXPECTED: INFO
    RISK: NONE
    GUARD: CYRILLIC_ER_FORM ≠ EFFECT
  SAFE_CASE_005:
  ...
  SAFE_CASE_006:
    ...
    CONTEXT: a phrase rich in "р", yet all Cyrillic
    EXPECTED: INFO
    RISK: NONE
    GUARD: CYRILLIC_ER_FORM ≠ AUTOMATICALLY_SPOOF
...
RISK_CASES:
  RISK_CASE_001:
    ...
    CONTEXT: IDN/brand spoof — the token looks like paypal.com but р/а are Cyrillic
    RISK: CRITICAL
    ATTACK: replacing Latin p (and a) with Cyrillic yields a visually identical domain the attacker registers
  ...
  RISK_CASE_002:
    ...
    RISK: CRITICAL
    ATTACK: the string is machine-≠ stripe.com, but a human sees no difference
  ...
  RISK_CASE_005:
    ...
    CONTEXT: a phishing email from the "same" payment provider
    RISK: HIGH
...
CONFUSABLES:
  CONFUSABLE_001:
    ...
    RISK: CRITICAL
    RULE: LATIN_P ≠ CYRILLIC_ER (the primary impersonation target; visually identical in many fonts)
  CONFUSABLE_002:
    VISIBLE_FORM: ρ
    CODEPOINT: U+03C1
    NAME: GREEK SMALL LETTER RHO
    RISK: HIGH
    RULE: GREEK_RHO ≠ CYRILLIC_ER (a third script with the same p-like shape; complicates detection)
  CONFUSABLE_003:
    VISIBLE_FORM: ⲣ
    CODEPOINT: U+2CA3
    NAME: COPTIC SMALL LETTER RO
    RISK: MEDIUM
    RULE: COPTIC_RO ≠ CYRILLIC_ER (another look-alike from a fourth script)
  CONFUSABLE_004:
    VISIBLE_FORM: 𝗉
    CODEPOINT: U+1D5C9
    NAME: MATHEMATICAL SANS-SERIF SMALL P
    RISK: MEDIUM
    RULE: MATH_SANS_P ≠ CYRILLIC_ER (a math-alphanumeric styled Latin p used to dodge simple filters)
  CONFUSABLE_005:
    VISIBLE_FORM: ｐ
    CODEPOINT: U+FF50
    NAME: FULLWIDTH LATIN SMALL LETTER P
    RISK: LOW
    RULE: FULLWIDTH_P ≠ CYRILLIC_ER (a full-width Latin p; a different compatibility form)
  CONFUSABLE_006:
    VISIBLE_FORM: ṗ
    CODEPOINT: U+1E57
    NAME: LATIN SMALL LETTER P WITH DOT ABOVE
    RISK: LOW
    RULE: LATIN_P_DOT_ABOVE ≠ CYRILLIC_ER (an accented Latin twin)

CONTRADICTION_GUARDS:
  CG1:
    TRIGGER: "the domain `раypal.com` is PayPal"
    RESPONSE: CYRILLIC_ER_FORM ≠ LATIN_P
    RULE: р/а are Cyrillic; the registrable domain is different — DNS decides, not the eye
  CG2:
...
PHAGO_ENTITY_MIMICRY:
  PE_001:
    ...
    RISK: CRITICAL
    NOTE: Cyrillic р (with а) mimics the NAME itself of a verified brand (not merely structure) — direct mimicry of an entity's existence. This is why the registry flags the sign PHAGO ●; commercial lookalike defenses often miss this class.
  PE_002:
...
CATEGORY_A: FORM_MANIPULATION (2)
  A1: substitution of Latin p (U+0070) with Cyrillic р (U+0440) in a brand domain
  A2: mixing Cyrillic р with Greek ρ / math sans-serif p to complicate detection
CATEGORY_B: CONTEXT_INJECTION (2)
  B1: Cyrillic р bypasses a Latin-keyword blocklist (sрam)
  B2: Cyrillic р in a mail domain (billing@раypro.example) for phishing
CATEGORY_C: SEQUENCE_MANIPULATION (2)
  C1: the mixed token `striрe` (SC1) — scripts within one word
  C2: multiple substitution `раyраl` (SC2) onto a target brand
CATEGORY_D: SEMANTIC_MIMICRY (2)
  D1: the handle `@suррort` imitates a service account
  D2: "shoр-official" — a look-alike in a pseudo-official name
CATEGORY_E: PHAGO_ENTITY_MIMICRY (2)
  E1: `раypal.com` — mimicry of a brand name (PE_001)
  E2: `@striрe_support` — mimicry of an official account (PE_002)
