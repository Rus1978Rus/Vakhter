...
DOCUMENT_ID: SIGN_CORE_CARD_LEFT_CURLY_BRACKET_U007B_GEN3_v0_3_EN
...
TRANSLATION_NOTE: English mirror of SIGN_CORE_CARD_LEFT_CURLY_BRACKET_U007B_GEN3_v0_3_RU (authoritative). Codepoints, field names and formulas identical. DRAFT for our work (Vakhter); conveyor is a separate project.
...
2. META
============================================================
CARD_UID: SIGN_CORE_CARD_LEFT_CURLY_BRACKET_U007B_GEN3_v0_3_EN
CODEPOINT: U+007B
VISIBLE_FORM: {
...
DISPLAY_NAME: left brace / template & object open
CATEGORY_ROADMAP: INJ (SSTI template, object/brace expansion) · PHAGO: — (structure open forgery)
...
SIGN_CATEGORY:
  - code block opener ({ ... })
  - JSON/dict/set object opener ({"k": "v"})
  - template expression opener ({{ expr }}, ${...}, {0})
  - shell brace expansion opener ({a,b})

WHAT_THIS_SIGN_IS_NOT:
  1. NOT_BLOCK_ONLY — "{" is not always a benign code/object block (it opens template expressions)
  2. NOT_TEMPLATE_SAFE — "{{" / "{%" opens an expression a template engine may evaluate (SSTI/RCE)
  3. NOT_OBJECT_SAFE — an object opener can inject keys/operators (NoSQL/JSON)
  4. NOT_ESCAPED_PROOF — the presence of "{" does not mean it is quoted/escaped
  5. NOT_ENCODED_SAFE — "%7B" may be decoded back to "{" later
  6. NOT_AUTHORITY — "{" does not confirm officialness
  7. NOT_EXECUTION_TRIGGER — by itself it executes nothing; context makes it open/evaluate
  8. NOT_TRUST_SIGNAL — it does not increase trust
  9. NOT_FORMAT_SAFE — "{0}"/"{name}" can leak or index into a format string
...
NOTE: "{" (ZONE_1) has parallel functions (code block, object, template expression, brace expansion) co-existing without cultural precession. Polysemy of a stable sign.
LAYER_ANOMALY:
...
RISK_CASES:
  RISK_CASE_001:
    ...
    CONTEXT: "{{" opening an expression evaluated by a template engine
    RISK: CRITICAL
    ATTACK: "{{...}}" is evaluated server-side, leaking config or running code (SSTI/RCE)
    GUARD: LEFT_CURLY_BRACKET_FORM ≠ TEMPLATE_SAFETY_PROOF
  RISK_CASE_002:
    NAME: NOSQL_OBJECT_INJECTION
    INPUT: '{"$gt": ""}'
    CONTEXT: "{" opening an injected NoSQL operator object
    RISK: HIGH
    ATTACK: "{" injects a "$gt" operator turning a value match into always-true (auth bypass)
    GUARD: LEFT_CURLY_BRACKET_FORM ≠ OBJECT_SAFETY_PROOF
  RISK_CASE_003:
    NAME: FORMAT_STRING_LEAK
    INPUT: "{0.__class__.__mro__} in a format string"
    CONTEXT: "{" opening a format field that walks object internals
    RISK: HIGH
    ATTACK: "{0...}" indexes into objects to leak secrets/globals via str.format
    GUARD: LEFT_CURLY_BRACKET_FORM ≠ FORMAT_SAFETY_PROOF
  RISK_CASE_004:
    NAME: SHELL_BRACE_EXPANSION
    INPUT: "cp file {a,../../etc/passwd}"
    CONTEXT: "{" opening a shell brace expansion into extra targets
    RISK: MEDIUM
    ATTACK: "{a,b}" expands one argument into several, reaching unintended paths
    GUARD: LEFT_CURLY_BRACKET_FORM ≠ EFFECT
  RISK_CASE_005:
    ...
    RISK: HIGH
    ATTACK: %7B decodes to "{" AFTER the check → template evaluation
    GUARD: LEFT_CURLY_BRACKET_FORM ≠ ENCODED_SAFETY_PROOF
  RISK_CASE_006:
    NAME: FULLWIDTH_BRACE_BYPASS
    INPUT: "｛｛7*7｝｝ (fullwidth ｛ U+FF5B)"
...
CONFUSABLES:
  CONFUSABLE_001:
    ...
    RISK: HIGH
    RULE: FULLWIDTH_LEFT_CURLY_BRACKET ≠ LEFT_CURLY_BRACKET (bypasses a filter looking for ASCII "{")
  CONFUSABLE_002:
    VISIBLE_FORM: ﹛
    CODEPOINT: U+FE5B
...
CONTRADICTION_GUARDS:
  CG1:
    TRIGGER: "'{' is always a benign code/object block"
    RESPONSE: LEFT_CURLY_BRACKET_FORM ≠ BLOCK_ONLY_PROOF
    RULE: "{{" / "{%" opens a template expression an engine may evaluate
  CG2:
    TRIGGER: "a template block is inert text"
    RESPONSE: LEFT_CURLY_BRACKET_FORM ≠ TEMPLATE_SAFETY_PROOF
    RULE: the template engine evaluates the expression (SSTI/RCE)
  CG3:
    TRIGGER: "a JSON object cannot be dangerous"
    RESPONSE: LEFT_CURLY_BRACKET_FORM ≠ OBJECT_SAFETY_PROOF
    RULE: "{" can inject a NoSQL operator ($gt/$ne) or forge a key
  CG4:
    TRIGGER: "'%7B' is safe forever"
  ...
  CG5:
    ...
    RULE: fullwidth ｛ (U+FF5B) and small ﹛ (U+FE5B) are different codepoints
  CG6:
...
SEQUENCE_LAYER_BOUNDARY:
  ...
  SEQUENCE_CANDIDATES:
    SC1:
      ...
      RISK_LEVEL: CRITICAL
      POSSIBLE_CONTEXTS: opening an evaluated template expression (SSTI)
      REQUIRES_SEQUENCE_INTEGRATOR: YES
    SC2:
      SEQUENCE: '{"$'
      NAME: NOSQL_OPERATOR_OPEN
      RISK_LEVEL: HIGH
      POSSIBLE_CONTEXTS: opening an injected NoSQL operator object
      REQUIRES_SEQUENCE_INTEGRATOR: YES
    SC3:
      SEQUENCE: "{%"
      NAME: TEMPLATE_TAG_OPEN
      RISK_LEVEL: HIGH
      POSSIBLE_CONTEXTS: opening a template control tag (if/for/include)
      REQUIRES_SEQUENCE_INTEGRATOR: YES
  IF NOT_APPLICABLE: not applicable — sequences with "{" are central to template/object injection.

PHAGO_ENTITY_MIMICRY:
  NOT_APPLICABLE:
    REASON: "{" opens a block/object/template expression, but does not imitate the existence of a verified entity. Its risks are template/object injection, not entity mimicry.
    REVIEW_REQUIRED: YES
...
CATEGORY_A: FORM_MANIPULATION (2)
  A1: substitution of ASCII "{" with fullwidth ｛ (U+FF5B) to bypass a filter
  A2: substitution with small ﹛ (U+FE5B)
CATEGORY_B: CONTEXT_INJECTION (2)
  B1: SSTI template injection {{7*7}}
  B2: NoSQL object injection {"$gt": ""}
CATEGORY_C: SEQUENCE_MANIPULATION (2)
  C1: "{{" (SC1) — template expression open
  C2: '{"$' (SC2) — NoSQL operator open
CATEGORY_D: SEMANTIC_MIMICRY (2)
  D1: "{" presented as a harmless JSON/code block inside a template field
  D2: "%7B" as "safe" encoded text with a later decode
CATEGORY_E: PHAGO_ENTITY_MIMICRY (2)
  E1: N/A — the sign is not a PHAGO carrier; vector: template expression into a rendered page
  E2: N/A — vector: operator-object injection into a NoSQL query
...
============================================================
MUTATION_01:
  CLAIM: "{" is always a benign code/object block
  EXPECTED: FAIL_BLOCK_ONLY_MIMICRY
  RESULT: FAIL
MUTATION_02:
  CLAIM: a template block is inert text
  EXPECTED: FAIL_TEMPLATE_SAFETY_MIMICRY
  RESULT: FAIL
MUTATION_03:
  CLAIM: a JSON object cannot be dangerous
  EXPECTED: FAIL_OBJECT_SAFETY_MIMICRY
  RESULT: FAIL
MUTATION_04:
  CLAIM: "%7B" is safe forever
...
============================================================
OQ1:
  QUESTION: how to escape "{" per context (template/JSON/shell/format) without false positives on code blocks/sets/CSS/regex quantifiers?
  STATUS: CLOSED_AS_DELEGATED_TO_INTEGRATOR (sandboxed/logic-less templating + schema-validated JSON + safe format APIs + brace-expansion disabling is an integrator/runtime concern)
