...
DOCUMENT_ID: SIGN_CORE_CARD_CYRILLIC_CAPITAL_A_U0410_GEN3_v0_3_RU
...
TRANSLATION_NOTE: Русский оригинал (авторитетный источник). Английский документ SIGN_CORE_CARD_CYRILLIC_CAPITAL_A_U0410_GEN3_v0_3_EN — зеркало. Кодовые точки, имена полей и формулы идентичны. Гомоглиф: базовый закон — ВЫГЛЯДИТ_ОДИНАКОВО ≠ ЯВЛЯЕТСЯ_ТЕМ_ЖЕ. ЧЕРНОВИК для нашей работы (Vakhter); конвейер — отдельный проект.
...
2. META
============================================================
CARD_UID: SIGN_CORE_CARD_CYRILLIC_CAPITAL_A_U0410_GEN3_v0_3_RU
CODEPOINT: U+0410
VISIBLE_FORM: А
...
DISPLAY_NAME: кириллическая «А» (гомоглиф латинской заглавной A)
...
SIGN_CATEGORY:
  - кириллическая заглавная буква (легитимна в русском / других кириллических письменностях)
  - гомоглиф латинской заглавной «A» (U+0041)
  - потенциальный носитель гомоглифного / IDN-спуфинга при смешении письменностей

WHAT_THIS_SIGN_IS_NOT:
  1. NOT_LATIN_CAP_A — кириллическая А (U+0410) НЕ есть латинская A (U+0041); другая кодовая точка
  2. NOT_SAME_STRING_AS_LATIN — строка с кириллической А не машинно-равна своему латинскому двойнику
  3. NOT_BRAND_NAME_PROOF — «Аmazon» не доказывает связь с брендом Amazon
...
NOTE: кириллическая «А» — стабильная буква без культурной прецессии функций. «Гомоглиф» — не эпоха, а свойство визуального совпадения с латинской A, сосуществующее с легитимной функцией буквы. Опасность контекстна (смешение письменностей), а не эпохальна — поэтому SEMANTIC_EPOCH_TRACKER не применяется.
LAYER_ANOMALY:
...
SAFE_CASES:
  ...
  SAFE_CASE_003:
    INPUT: "Азовское море"
    CONTEXT: русские слова, где «А» — обычная заглавная
    EXPECTED: INFO
    RISK: NONE
    GUARD: CYRILLIC_CAP_A_FORM ≠ AUTOMATICALLY_SPOOF
  SAFE_CASE_004:
    INPUT: "Астрахань и Анапа"
  ...
  SAFE_CASE_006:
    INPUT: "Анна и Андрей"
    CONTEXT: два имени на «А», всё кириллица
    EXPECTED: INFO
    RISK: NONE
    GUARD: CYRILLIC_CAP_A_FORM ≠ AUTOMATICALLY_SPOOF
...
RISK_CASES:
  RISK_CASE_001:
    ...
    INPUT: "Аmazon.com"
    CONTEXT: IDN/бренд-спуф — токен выглядит как Amazon.com, но первый символ кириллический
    RISK: CRITICAL
    ATTACK: замена латинской A на кириллическую А даёт визуально идентичный бренд, который регистрирует атакующий
    GUARD: CYRILLIC_CAP_A_FORM ≠ LATIN_CAP_A
  RISK_CASE_002:
    NAME: ALLCAPS_BRAND_SUBSTITUTION
    INPUT: "АDOBE-support.com"
  ...
  RISK_CASE_003:
    ...
    RISK: HIGH
    ATTACK: похожий хэндл выглядит как Apple_care, но это другой аккаунт
  ...
  RISK_CASE_005:
    ...
    INPUT: "no-reply@Аcme-billing.example"
  ...
  RISK_CASE_006:
    ...
    INPUT: "АLIBАBА"
    CONTEXT: несколько подмен в одном капслок-токене
    RISK: HIGH
...
CONFUSABLES:
  CONFUSABLE_001:
    ...
    RISK: CRITICAL
    RULE: LATIN_CAP_A ≠ CYRILLIC_CAP_A (главная цель имперсонации; визуально идентична во многих шрифтах)
  CONFUSABLE_002:
    VISIBLE_FORM: Α
    CODEPOINT: U+0391
    NAME: GREEK CAPITAL LETTER ALPHA
    RISK: HIGH
    RULE: GREEK_CAP_ALPHA ≠ CYRILLIC_CAP_A (третий скрипт с той же формой A; усложняет детекцию)
  CONFUSABLE_003:
    VISIBLE_FORM: 𝗔
    CODEPOINT: U+1D5D4
    NAME: MATHEMATICAL SANS-SERIF BOLD CAPITAL A
    RISK: MEDIUM
    RULE: MATH_SANS_CAP_A ≠ CYRILLIC_CAP_A (математически-стилизованная латинская A для обхода простых фильтров)
  CONFUSABLE_004:
    VISIBLE_FORM: Ａ
    CODEPOINT: U+FF21
    NAME: FULLWIDTH LATIN CAPITAL LETTER A
    RISK: MEDIUM
    RULE: FULLWIDTH_CAP_A ≠ CYRILLIC_CAP_A (полноширинная латинская A; другая форма совместимости)
  CONFUSABLE_005:
    VISIBLE_FORM: Å
    CODEPOINT: U+00C5
    NAME: LATIN CAPITAL LETTER A WITH RING ABOVE
    RISK: LOW
    RULE: LATIN_CAP_A_RING ≠ CYRILLIC_CAP_A (латинский двойник с диакритикой)
  CONFUSABLE_006:
    VISIBLE_FORM: Ą
    CODEPOINT: U+0104
    NAME: LATIN CAPITAL LETTER A WITH OGONEK
    RISK: LOW
    RULE: LATIN_CAP_A_OGONEK ≠ CYRILLIC_CAP_A (декорированный латинский двойник)

CONTRADICTION_GUARDS:
  CG1:
    TRIGGER: «бренд `Аmazon` — это Amazon»
    RESPONSE: CYRILLIC_CAP_A_FORM ≠ LATIN_CAP_A
    RULE: первый символ кириллический; регистрируемый домен/имя другой — решает DNS, а не глаз
  CG2:
  ...
  CG3:
    TRIGGER: «любая кириллическая А в тексте — атака»
  ...
  CG4:
    ...
    RULE: кириллическая А вне ASCII; латинский фильтр её не сматчит
  CG5:
    TRIGGER: «хэндл `@Аpple_care` — тот же аккаунт, что и @Apple_care»
    RESPONSE: CYRILLIC_CAP_A_FORM ≠ VERIFICATION
    RULE: визуальное сходство не идентифицирует аккаунт
  CG6:
    TRIGGER: «замена A→А в идентификаторе безвредна»
...
SEQUENCE_LAYER_BOUNDARY:
  ...
  SEQUENCE_CANDIDATES:
    SC1:
      SEQUENCE: "Аmazon" (кириллическая А + латиница в одном токене)
    ...
    SC2:
      SEQUENCE: "АLIBАBА" (несколько кириллических среди латинских заглавных)
...
PHAGO_ENTITY_MIMICRY:
  PE_001:
    ...
    RISK: CRITICAL
    NOTE: кириллическая А имитирует само ИМЯ верифицированного бренда (а не только структуру) — прямая имитация существования сущности. Поэтому реестр помечает знак PHAGO ●; коммерческие защиты от двойников этот класс часто пропускают.
  PE_002:
    INPUT: "@Аdobe_official"
    TYPE: PHAGO_ENTITY_MIMICRY
    RISK: HIGH
    NOTE: имперсонация официального бренд-аккаунта через двойника в имени.
...
CATEGORY_A: FORM_MANIPULATION (2)
  A1: замена латинской A (U+0041) на кириллическую А (U+0410) в домене бренда
  A2: смешение кириллической А с греческой Alpha Α / математической sans-serif A для усложнения детекции
CATEGORY_B: CONTEXT_INJECTION (2)
  B1: кириллическая А обходит латинский блок-лист ключевых слов (АDMIN)
  B2: кириллическая А в почтовом домене (no-reply@Аcme-billing.example) для фишинга
CATEGORY_C: SEQUENCE_MANIPULATION (2)
  C1: смешанный токен `Аmazon` (SC1) — письменности внутри одного слова
  C2: множественная подмена `АLIBАBА` (SC2) на целевой бренд
CATEGORY_D: SEMANTIC_MIMICRY (2)
  D1: хэндл `@Аpple_care` имитирует бренд-аккаунт
  D2: "Аcme-official" — двойник в псевдо-официальном имени
CATEGORY_E: PHAGO_ENTITY_MIMICRY (2)
  E1: `Аmazon.com` — имитация имени бренда (PE_001)
  E2: `@Аdobe_official` — имитация официального аккаунта (PE_002)
...
============================================================
MUTATION_01:
  CLAIM: `Аmazon.com` с кириллической А — это домен Amazon
  EXPECTED: FAIL_BRAND_NAME_MIMICRY
  RESULT: FAIL
MUTATION_02:
  CLAIM: строка с кириллической А машинно-равна её латинскому написанию
...
MUTATION_03:
  ...
  RESULT: FAIL
MUTATION_04:
  CLAIM: ASCII-фильтр на "ADMIN" поймает "АDMIN"
  EXPECTED: FAIL_ASCII_COVERAGE_MIMICRY
  RESULT: FAIL
MUTATION_05:
  CLAIM: хэндл `@Аpple_care` — тот же аккаунт, что и `@Apple_care`
  EXPECTED: FAIL_IDENTITY_MIMICRY
  RESULT: FAIL
MUTATION_06:
  CLAIM: замена A→А в идентификаторе безвредна
