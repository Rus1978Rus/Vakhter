...
DOCUMENT_ID: SIGN_CORE_CARD_CYRILLIC_CAPITAL_HA_U0425_GEN3_v0_3_EN
...
TRANSLATION_NOTE: English mirror of SIGN_CORE_CARD_CYRILLIC_CAPITAL_HA_U0425_GEN3_v0_3_RU (authoritative). Codepoints, field names and formulas identical. Homoglyph sign: core law is LOOKS_SAME ≠ IS_SAME. DRAFT for our work (Vakhter); conveyor is a separate project.
...
2. META
============================================================
CARD_UID: SIGN_CORE_CARD_CYRILLIC_CAPITAL_HA_U0425_GEN3_v0_3_EN
CODEPOINT: U+0425
VISIBLE_FORM: Х
...
DISPLAY_NAME: Cyrillic "Х" (homoglyph of Latin capital X; reads /kh/)
...
SIGN_CATEGORY:
  - Cyrillic capital letter Ha (sounds /kh/, NOT /ks/; legitimate in Russian / other Cyrillic scripts)
  - homoglyph of Latin capital "X" (U+0058)
  - potential carrier of homoglyph / IDN spoofing when scripts are mixed

WHAT_THIS_SIGN_IS_NOT:
  1. NOT_LATIN_CAP_X — Cyrillic Х (U+0425) is NOT Latin X (U+0058); a different codepoint AND a different sound (/kh/ not /ks/)
  2. NOT_SAME_STRING_AS_LATIN — a string with Cyrillic Х is not machine-equal to its Latin twin
  3. NOT_BRAND_NAME_PROOF — "ХBOX" does not prove a link to the Xbox brand
...
NOTE: Cyrillic "Х" is a stable letter with no cultural precession of functions. The X-shape / KH-sound mismatch is a permanent trait, not an epoch — the letter reads /kh/ in Cyrillic while looking like Latin X. The danger is contextual (mixed scripts), not epochal — so SEMANTIC_EPOCH_TRACKER does not apply.
LAYER_ANOMALY:
...
SAFE_CASES:
  SAFE_CASE_001:
    INPUT: "Холодная зима" (a cold winter)
    CONTEXT: ordinary Russian text (single script)
    EXPECTED: INFO
    RISK: NONE
    GUARD: CYRILLIC_CAP_HA_FORM ≠ AUTOMATICALLY_SPOOF
  SAFE_CASE_002:
    INPUT: "Хабаровск и Химки" (Khabarovsk and Khimki)
    CONTEXT: Russian place names
    EXPECTED: INFO
    RISK: NONE
    GUARD: CYRILLIC_CAP_HA_FORM ≠ EFFECT
  SAFE_CASE_003:
    INPUT: "Хлеб и соль" (bread and salt)
    CONTEXT: Russian words where "Х" is an ordinary capital
    EXPECTED: INFO
    RISK: NONE
    GUARD: CYRILLIC_CAP_HA_FORM ≠ AUTOMATICALLY_SPOOF
  SAFE_CASE_004:
    INPUT: "Художественный музей" (an art museum)
    CONTEXT: single-script Cyrillic phrase
    EXPECTED: INFO
    RISK: NONE
    GUARD: CYRILLIC_CAP_HA_FORM ≠ EFFECT
  SAFE_CASE_005:
    INPUT: "Хорошая погода" (nice weather)
    CONTEXT: Russian words beginning with "Х"
    EXPECTED: INFO
    RISK: NONE
    GUARD: CYRILLIC_CAP_HA_FORM ≠ AUTOMATICALLY_SPOOF
  SAFE_CASE_006:
    INPUT: "Христина и Харитон" (Khristina and Khariton)
    CONTEXT: two names beginning with "Х", all Cyrillic
    EXPECTED: INFO
    RISK: NONE
    GUARD: CYRILLIC_CAP_HA_FORM ≠ AUTOMATICALLY_SPOOF
...
RISK_CASES:
  RISK_CASE_001:
    ...
    INPUT: "ХBOX-live-login.com" (Cyrillic Х at the start of a brand)
    CONTEXT: IDN/brand spoof — the token looks like XBOX but the first char is Cyrillic
    RISK: CRITICAL
    ATTACK: replacing Latin X with Cyrillic Х yields a visually identical brand the attacker registers
    GUARD: CYRILLIC_CAP_HA_FORM ≠ LATIN_CAP_X
  RISK_CASE_002:
    NAME: ALLCAPS_BRAND_SUBSTITUTION
    INPUT: "ХEROX-support.com" (Cyrillic Х in an all-caps brand)
  ...
  RISK_CASE_003:
    ...
    INPUT: "@ХcorpSupport" (Cyrillic Х in a handle)
    CONTEXT: impersonating a brand account in a chat/social platform
    RISK: HIGH
    ATTACK: the look-alike handle appears as XcorpSupport but is a different account
    GUARD: CYRILLIC_CAP_HA_FORM ≠ VERIFICATION
  RISK_CASE_004:
    NAME: FILTER_BYPASS_KEYWORD
    INPUT: "ХSS" (Cyrillic Х bypasses an "XSS" blocklist)
  ...
  RISK_CASE_005:
    ...
    INPUT: "no-reply@Хfinity-billing.example" (Cyrillic Х in the mail domain)
  ...
  RISK_CASE_006:
    ...
    INPUT: "МAХ" (Cyrillic М and Х around a Latin A, imitating MAX)
    CONTEXT: multiple substitutions in one all-caps token (М and Х Cyrillic, A Latin — a script mix)
    RISK: HIGH
...
CONFUSABLES:
  CONFUSABLE_001:
    ...
    RISK: CRITICAL
    RULE: LATIN_CAP_X ≠ CYRILLIC_CAP_HA (the primary impersonation target; visually identical in many fonts)
  CONFUSABLE_002:
    VISIBLE_FORM: Χ
    CODEPOINT: U+03A7
    NAME: GREEK CAPITAL LETTER CHI
    RISK: HIGH
    RULE: GREEK_CAP_CHI ≠ CYRILLIC_CAP_HA (a third script with the same X-shape; complicates detection)
  CONFUSABLE_003:
    VISIBLE_FORM: 𝗫
    CODEPOINT: U+1D5EB
    NAME: MATHEMATICAL SANS-SERIF BOLD CAPITAL X
    RISK: MEDIUM
    RULE: MATH_SANS_CAP_X ≠ CYRILLIC_CAP_HA (a math-alphanumeric styled Latin X used to dodge simple filters)
  CONFUSABLE_004:
    VISIBLE_FORM: Ｘ
    CODEPOINT: U+FF38
    NAME: FULLWIDTH LATIN CAPITAL LETTER X
    RISK: MEDIUM
    RULE: FULLWIDTH_CAP_X ≠ CYRILLIC_CAP_HA (a full-width Latin X; a different compatibility form)
  CONFUSABLE_005:
    VISIBLE_FORM: ✕
    CODEPOINT: U+2715
    NAME: MULTIPLICATION X
    RISK: LOW
    RULE: MULTIPLICATION_X ≠ CYRILLIC_CAP_HA (a symbol shaped like X; not a letter)
  CONFUSABLE_006:
    VISIBLE_FORM: Ⲭ
    CODEPOINT: U+2CAC
    NAME: COPTIC CAPITAL LETTER KHI
    RISK: LOW
    RULE: COPTIC_KHI ≠ CYRILLIC_CAP_HA (a Coptic letter with the same X-shape)

CONTRADICTION_GUARDS:
  CG1:
    TRIGGER: "the brand `ХBOX` is Xbox"
    RESPONSE: CYRILLIC_CAP_HA_FORM ≠ LATIN_CAP_X
    RULE: the first char is Cyrillic Ha (/kh/); the registrable domain/name is different — DNS decides, not the eye
  CG2:
  ...
  CG5:
    TRIGGER: "the handle `@ХcorpSupport` is the same account as @XcorpSupport"
...
SEQUENCE_LAYER_BOUNDARY:
  ...
  SEQUENCE_CANDIDATES:
    ...
    SC2:
      SEQUENCE: "МAХ" (Cyrillic М and Х around a Latin A)
      NAME: MULTI_HOMOGLYPH_TOKEN
      RISK_LEVEL: HIGH
      POSSIBLE_CONTEXTS: multiple substitution onto a target all-caps token
      REQUIRES_SEQUENCE_INTEGRATOR: YES
...
CATEGORY_A: FORM_MANIPULATION (2)
  A1: substitution of Latin X (U+0058) with Cyrillic Х (U+0425) in a brand domain
  A2: mixing Cyrillic Х with Greek Chi Χ / math sans-serif X to complicate detection
CATEGORY_B: CONTEXT_INJECTION (2)
  B1: Cyrillic Х bypasses a Latin-token blocklist (ХSS)
  B2: Cyrillic Х in a mail domain (no-reply@Хfinity-billing.example) for phishing
CATEGORY_C: SEQUENCE_MANIPULATION (2)
  C1: the mixed token `ХBOX` (SC1) — scripts within one word
  C2: multiple substitution `МAХ` (SC2) onto a target token
CATEGORY_D: SEMANTIC_MIMICRY (2)
  D1: the handle `@ХcorpSupport` imitates a brand account
  D2: "Хfinity-billing" — a look-alike in a pseudo-official name
CATEGORY_E: PHAGO_ENTITY_MIMICRY (2)
  E1: `ХBOX-live-login.com` — mimicry of a brand name (PE_001)
  E2: `@ХcorpSupport` — mimicry of an official account (PE_002)
...
============================================================
MUTATION_01:
  CLAIM: `ХBOX-live-login.com` with Cyrillic Х is Xbox's domain
...
MUTATION_03:
  ...
  RESULT: FAIL
MUTATION_04:
  CLAIM: an ASCII filter on "XSS" will catch "ХSS"
  EXPECTED: FAIL_ASCII_COVERAGE_MIMICRY
  RESULT: FAIL
MUTATION_05:
  CLAIM: the handle `@ХcorpSupport` is the same account as `@XcorpSupport`
