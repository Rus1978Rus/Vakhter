...
DOCUMENT_ID: SIGN_CORE_CARD_ROMAN_CAPITAL_I_U2160_GEN3_v0_3_EN
...
TRANSLATION_NOTE: English mirror of SIGN_CORE_CARD_ROMAN_CAPITAL_I_U2160_GEN3_v0_3_RU (authoritative). Codepoints, field names and formulas identical. Homoglyph sign: core law is LOOKS_SAME ≠ IS_SAME. DRAFT for our work (Vakhter); conveyor is a separate project.
...
2. META
============================================================
CARD_UID: SIGN_CORE_CARD_ROMAN_CAPITAL_I_U2160_GEN3_v0_3_EN
CODEPOINT: U+2160
VISIBLE_FORM: Ⅰ
...
DISPLAY_NAME: Roman numeral "Ⅰ" one (homoglyph of Latin capital I)
...
SIGN_CATEGORY:
  - Roman-numeral letter-form "roman numeral one" (means 1 as a standalone numeral; a Number Forms character)
  - homoglyph of Latin capital "I" (U+0049)
  - potential carrier of homoglyph spoofing when mixed INTO a Latin word

WHAT_THIS_SIGN_IS_NOT:
  1. NOT_LATIN_CAP_I — Roman Ⅰ (U+2160) is NOT Latin I (U+0049); a different codepoint AND a numeral (value 1), not a letter
  2. NOT_SAME_STRING_AS_LATIN — a string with Roman Ⅰ is not machine-equal to its Latin twin
  3. NOT_BRAND_NAME_PROOF — "ⅠBM" does not prove a link to the IBM brand
...
SAFE_CASES:
  ...
  SAFE_CASE_002:
    INPUT: "World War Ⅰ" (WWI, Roman numeral)
  ...
  SAFE_CASE_004:
    ...
    GUARD: ROMAN_CAP_I_FORM ≠ EFFECT
  SAFE_CASE_005:
    INPUT: "Louis Ⅰ" (Louis the First, Roman numeral)
    CONTEXT: a standalone Roman numeral token
    EXPECTED: INFO
    RISK: NONE
    GUARD: ROMAN_CAP_I_FORM ≠ AUTOMATICALLY_SPOOF
  SAFE_CASE_006:
    INPUT: "Ⅰ ⅠⅠ ⅠⅠⅠ" (1 2 3, Roman numerals)
...
RISK_CASES:
  RISK_CASE_001:
    ...
    INPUT: "ⅠBM-support.com" (Roman Ⅰ at the start of a brand)
    CONTEXT: IDN/brand spoof — the token looks like IBM but the first char is a Roman numeral
    RISK: CRITICAL
  ...
  RISK_CASE_002:
    ...
    INPUT: "ⅠRS-refund-alert.com" (Roman Ⅰ in an all-caps brand)
  ...
  RISK_CASE_003:
    ...
    INPUT: "@ⅠntelSupport" (Roman Ⅰ in a handle)
    CONTEXT: impersonating a brand account in a chat/social platform
    RISK: HIGH
    ATTACK: the look-alike handle appears as IntelSupport but is a different account
    GUARD: ROMAN_CAP_I_FORM ≠ VERIFICATION
  RISK_CASE_004:
    NAME: FILTER_BYPASS_KEYWORD
    INPUT: "ⅠNFO" (Roman Ⅰ bypasses an "INFO" blocklist)
  ...
  RISK_CASE_005:
    ...
    INPUT: "billing@ⅠNTUIT-invoice.example" (Roman Ⅰ in the mail domain)
  ...
  RISK_CASE_006:
    ...
    INPUT: "ⅠBⅠS" (two Roman Ⅰ around Latin letters, imitating IBIS)
    CONTEXT: multiple substitutions in one all-caps token (Ⅰ Roman, B/S Latin — a numeral-in-word mix)
    RISK: HIGH
...
CONFUSABLES:
  CONFUSABLE_001:
    ...
    RISK: CRITICAL
    RULE: LATIN_CAP_I ≠ ROMAN_CAP_I (the primary impersonation target; visually identical in many fonts)
  CONFUSABLE_002:
    VISIBLE_FORM: Ι
    CODEPOINT: U+0399
    NAME: GREEK CAPITAL LETTER IOTA
    RISK: HIGH
    RULE: GREEK_CAP_IOTA ≠ ROMAN_CAP_I (another script with the same I-shape; complicates detection)
  CONFUSABLE_003:
    VISIBLE_FORM: Ｉ
    CODEPOINT: U+FF29
    NAME: FULLWIDTH LATIN CAPITAL LETTER I
    RISK: MEDIUM
    RULE: FULLWIDTH_CAP_I ≠ ROMAN_CAP_I (a full-width Latin I; a different compatibility form)
  CONFUSABLE_004:
    VISIBLE_FORM: 𝗜
    CODEPOINT: U+1D5DC
    NAME: MATHEMATICAL SANS-SERIF BOLD CAPITAL I
    RISK: MEDIUM
    RULE: MATH_SANS_CAP_I ≠ ROMAN_CAP_I (a math-alphanumeric styled Latin I used to dodge simple filters)
  CONFUSABLE_005:
    VISIBLE_FORM: 𝐈
    CODEPOINT: U+1D408
    NAME: MATHEMATICAL BOLD CAPITAL I
    RISK: LOW
    RULE: MATH_BOLD_CAP_I ≠ ROMAN_CAP_I (a bold-styled Latin I)
  CONFUSABLE_006:
    VISIBLE_FORM: 𝐼
    CODEPOINT: U+1D43C
    NAME: MATHEMATICAL ITALIC CAPITAL I
    RISK: LOW
    RULE: MATH_ITALIC_CAP_I ≠ ROMAN_CAP_I (an italic-styled Latin I)

CONTRADICTION_GUARDS:
  CG1:
    TRIGGER: "the brand `ⅠBM` is IBM"
  ...
  CG5:
    TRIGGER: "the handle `@ⅠntelSupport` is the same account as @IntelSupport"
...
SEQUENCE_LAYER_BOUNDARY:
  ...
  SEQUENCE_CANDIDATES:
    SC1:
      SEQUENCE: "ⅠBM" (Roman Ⅰ + Latin in one token)
    ...
    SC2:
      SEQUENCE: "ⅠBⅠS" (two Roman Ⅰ among Latin capitals)
...
CATEGORY_A: FORM_MANIPULATION (2)
  A1: substitution of Latin I (U+0049) with Roman Ⅰ (U+2160) in a brand domain
  A2: mixing Roman Ⅰ with Greek Iota Ι / math sans-serif I to complicate detection
CATEGORY_B: CONTEXT_INJECTION (2)
  B1: Roman Ⅰ bypasses a Latin-keyword blocklist (ⅠNFO)
  B2: Roman Ⅰ in a mail domain (billing@ⅠNTUIT-invoice.example) for phishing
CATEGORY_C: SEQUENCE_MANIPULATION (2)
  C1: the numeral-in-word token `ⅠBM` (SC1) — a Roman form inside one word
  C2: multiple substitution `ⅠBⅠS` (SC2) onto a target brand
CATEGORY_D: SEMANTIC_MIMICRY (2)
  D1: the handle `@ⅠntelSupport` imitates a brand account
  D2: "ⅠNTUIT-invoice" — a look-alike in a pseudo-official name
CATEGORY_E: PHAGO_ENTITY_MIMICRY (2)
  E1: `ⅠBM-support.com` — mimicry of a brand name (PE_001)
  E2: `@ⅠntelSupport` — mimicry of an official account (PE_002)
...
============================================================
MUTATION_01:
  CLAIM: `ⅠBM-support.com` with Roman Ⅰ is IBM's domain
...
MUTATION_03:
  ...
  RESULT: FAIL
MUTATION_04:
  CLAIM: an ASCII filter on "INFO" will catch "ⅠNFO"
  EXPECTED: FAIL_ASCII_COVERAGE_MIMICRY
  RESULT: FAIL
MUTATION_05:
  CLAIM: the handle `@ⅠntelSupport` is the same account as `@IntelSupport`
